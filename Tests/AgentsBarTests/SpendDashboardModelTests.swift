...
struct SpendDashboardModelTests {
    ...
    func `count labels avoid plural agreement and localize numbers`() {
        ...
        AgentsBarLocalizationOverride.$appLanguage.withValue("fa") {
            #expect(agentsBarLocalizedInteger(12) == "۱۲")
    ...
    func `Codex account indices use app locale numerals`() throws {
        ...
        let persian = AgentsBarLocalizationOverride.$appLanguage.withValue("fa") {
        ...
        let arabic = AgentsBarLocalizationOverride.$appLanguage.withValue("ar") {
            SpendDashboardSource.codexRequest(
                account: account,
                homePath: home.path,
                providerName: "Codex",
