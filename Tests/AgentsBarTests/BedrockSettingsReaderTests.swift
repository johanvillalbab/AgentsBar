...
struct BedrockSettingsReaderTests {
    ...
    func `explicit profile auth mode wins`() {
        let env = ["AGENTSBAR_BEDROCK_AUTH_MODE": "profile"]
    ...
    func `hasCredentials in profile mode requires a profile name`() {
        let withProfile = ["AGENTSBAR_BEDROCK_AUTH_MODE": "profile", "AWS_PROFILE": "work"]
        let withoutProfile = ["AGENTSBAR_BEDROCK_AUTH_MODE": "profile"]
