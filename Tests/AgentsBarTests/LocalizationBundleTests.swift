...
struct LocalizationBundleTests {
    ...
    func `packaged app resolves localization bundle from resources`() throws {
        ...
        let bundle = agentsBarLocalizationResourceBundle(mainBundle: fixture.appBundle)

        #expect(bundle.bundleURL.lastPathComponent == "AgentsBar_AgentsBar.bundle")
    ...
    func `empty localized values fall back to English`() throws {
        ...
        let resourceBundle = agentsBarLocalizationResourceBundle(mainBundle: fixture.appBundle)
        let zhPath = try #require(resourceBundle.path(forResource: "zh-Hans", ofType: "lproj"))
        let zhBundle = try #require(Bundle(path: zhPath))

        #expect(agentsBarLocalizedString("Settings", bundle: zhBundle, resourceBundle: resourceBundle) == "Settings")
        #expect(agentsBarLocalizedString("Missing", bundle: zhBundle, resourceBundle: resourceBundle) == "Missing")
    }
        ...
        includeEmptyChineseLocalization: Bool = false) throws -> (root: URL, appBundle: Bundle)
    {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "agentsbar-localization-\(UUID().uuidString)",
            isDirectory: true)
        ...
        <dict>
            ...
            <key>CFBundleIdentifier</key><string>com.steipete.agentsbar.tests</string>
        ...
        if includeLocalizationBundle {
            let bundleURL = resourcesURL.appendingPathComponent("AgentsBar_AgentsBar.bundle", isDirectory: true)
