...
struct CLIServeAuthTests {
    ...
    func `serve help documents dashboard token transport honestly`() {
        ...
        #expect(serve.contains("AGENTSBAR_DASHBOARD_TOKEN"))
    ...
    func `serve host option parses and normalizes`() {
        #expect(AgentsBarCLI.decodeServeHost(from: ParsedValues(
            positional: [],
            options: [:],
            flags: [])) == "127.0.0.1")
        #expect(AgentsBarCLI.decodeServeHost(from: ParsedValues(
    ...
    func `serve flags parse through the real commander signature`() throws {
        ...
        #expect(AgentsBarCLI.decodeServeHost(from: values) == "0.0.0.0")
        #expect(AgentsBarCLI.decodeServeAllowPlainHTTP(from: values))
        #expect(AgentsBarCLI.resolveDashboardToken(from: values, environment: [:]) == .token("secret"))
        #expect(AgentsBarCLI.decodeServeHost(from: defaults) == "127.0.0.1")
        #expect(!AgentsBarCLI.decodeServeAllowPlainHTTP(from: defaults))
        #expect(AgentsBarCLI.resolveDashboardToken(from: defaults, environment: [:]) == .absent)
    }
    ...
    func `dashboard token resolution prefers the environment and rejects blanks`() {
        ...
        let envOverride = Dictionary(uniqueKeysWithValues: [
            (AgentsBarCLI.dashboardTokenEnvironmentVariable, "ENV_VALUE"),
        ])

        #expect(AgentsBarCLI.resolveDashboardToken(
            from: emptyValues,
            environment: [:]) == .absent)
        #expect(AgentsBarCLI.resolveDashboardToken(
            from: flagValues,
        ...
        #expect(AgentsBarCLI.resolveDashboardToken(
            from: emptyValues,
            environment: ["AGENTSBAR_DASHBOARD_TOKEN": "  "])
            == .empty(source: "AGENTSBAR_DASHBOARD_TOKEN"))
        #expect(AgentsBarCLI.resolveDashboardToken(
    ...
    func `serve startup validation enforces the token and plain-http matrix`() {
        ...
        #expect(AgentsBarCLI.validateServeStartup(
            host: "127.0.0.1",
    ...
    func `adding no-store preserves an existing cache-control header`() {
        ...
        let annotated = AgentsBarCLI.addingNoStore(plain)
        let untouched = AgentsBarCLI.addingNoStore(declared)

        #expect(annotated.extraHeaders.contains { $0 == ("Cache-Control", "no-store") })
        #expect(untouched.extraHeaders.count == 1)
        #expect(AgentsBarCLI.addingNoStore(annotated).extraHeaders.count == 1)
    }

    @Test
    func `unauthorized response advertises bearer challenge and no-store`() {
        let response = AgentsBarCLI.serveUnauthorizedResponse()
