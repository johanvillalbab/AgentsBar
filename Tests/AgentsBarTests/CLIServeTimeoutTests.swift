...
struct CLIServeTimeoutTests {
    ...
    func `serve cost keeps pricing refresh outside the request deadline`() {
        #expect(AgentsBarCLI.serveCostRefreshesPricingInBackground)
    }

    @Test
    func `serve deadlines clamp once from request entry`() throws {
        #expect(AgentsBarCLI.clampedServeRequestTimeout(.greatestFiniteMagnitude) == 86400)
        #expect(AgentsBarCLI.clampedServeRequestTimeout(1e308) == 86400)
        #expect(AgentsBarCLI.clampedServeRequestTimeout(-5) == 0)

        let startedAt = ContinuousClock().now
        let deadline = try #require(AgentsBarCLI.serveRequestDeadline(
            startedAt: startedAt,
            requestTimeout: .greatestFiniteMagnitude))
        #expect(startedAt.duration(to: deadline) == .seconds(86400))
        #expect(AgentsBarCLI.serveRequestDeadline(startedAt: startedAt, requestTimeout: 0) == nil)

        let requestDeadline = startedAt.advanced(by: .seconds(40))
        #expect(AgentsBarCLI.serveCostProviderDeadline(
            startedAt: startedAt,
            providerTimeout: 30,
    ...
    func `provider timeout preserves healthy rows and cannot stack provider work`() async {
        ...
        let first = Task {
            await AgentsBarCLI.serveCollectUsageOutputs(
    ...
    func `cost route variants cannot stack the same provider scan`() async {
        let clock = ServeManualDeadlineClock()
        let late = AgentsBarCLI.makeCostPayload(provider: .claude, snapshot: nil, error: nil)
        ...
        let first = Task {
            await AgentsBarCLI.serveCollectCostPayloads(
            ...
            { provider in
                ...
                return AgentsBarCLI.makeCostPayload(provider: provider, snapshot: nil, error: nil)
            }
        }
        await blocked.waitForStarts(1)
        await clock.waitForPendingSleeps(1)
