...
struct CLIHooksTests {
    ...
    func `sample quota-low event matches maximum threshold`() {
        let event = AgentsBarCLI.sampleHookEvent(type: .quotaLow, provider: UsageProvider.codex.rawValue)
    ...
    func `sample refresh failure uses production status`() {
        let event = AgentsBarCLI.sampleHookEvent(type: .refreshFailed, provider: UsageProvider.codex.rawValue)
    ...
    func `hook test JSON result is structured`() throws {
        let result = HookTestResult(
            ...
            stdout: "ok",
            error: nil)
        let encoded = try #require(AgentsBarCLI.encodeJSON([result], pretty: false))
