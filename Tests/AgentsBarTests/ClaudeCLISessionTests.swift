...
struct ClaudeCLISessionTests {
    ...
    func `probe launch reuses one persisted session identifier`() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("agentsbar-claude-session-\(UUID().uuidString)", isDirectory: true)
        ...
        let file = directory.appendingPathComponent(".agentsbar-session-id")
    ...
    func `unwritable probe directory keeps one process local fallback identifier`() {
        let directory = URL(fileURLWithPath: "/dev/null/AgentsBar-ClaudeProbe", isDirectory: true)
