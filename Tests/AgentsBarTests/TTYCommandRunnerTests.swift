...
struct TTYCommandRunnerEnvTests {
    ...
    func `codex status probe uses non persistent thread storage`() {
        let stateHome = URL(fileURLWithPath: "/tmp/agentsbar status \"state\"", isDirectory: true)
        ...
        #expect(args.contains("experimental_thread_store={type=\"in_memory\",id=\"agentsbar-status\"}"))
        #expect(args.contains("sqlite_home=\"/tmp/agentsbar status \\\"state\\\"\""))
    }
    ...
    func `sets working directory when provided`() throws {
        ...
        let dir = fm.temporaryDirectory.appendingPathComponent("agentsbar-tty-\(UUID().uuidString)", isDirectory: true)
