...
struct KeychainPromptSafetyAuditTests {
    ...
    func `default test runner explicitly suppresses real keychain access`() throws {
        ...
        #expect(script.contains("AGENTSBAR_ALLOW_TEST_KEYCHAIN_ACCESS"))
        #expect(script.contains("export AGENTSBAR_SUPPRESS_TEST_KEYCHAIN_ACCESS=1"))
    }

    @Test
    func `live TTY integration tests are opt in`() throws {
        let ttyTests = try Self.readRepoFile("Tests/AgentsBarTests/TTYIntegrationTests.swift")
    ...
    func `tests do not call Security item APIs except no UI query coverage`() throws {
        ...
        let offenders = try Self.swiftTestFiles().filter { file in
            ...
            return securityItemCalls.contains(where: text.contains)
                && !file.path.hasSuffix("Tests/AgentsBarTests/KeychainNoUIQueryTests.swift")
                && !file.path.hasSuffix("Tests/AgentsBarTests/KeychainPromptSafetyAuditTests.swift")
        }
    ...
    func `production source routes Security item APIs through the test safety gateway`() throws {
        ...
        let offenders = try Self.swiftFiles(
            ...
            .filter { file in
                guard !file.path.hasSuffix("Sources/AgentsBarCore/KeychainSecurity.swift") else { return false }
                let text = try Self.readFile(file)
                return securityItemCalls.contains(where: text.contains)
            }
    ...
    private static func swiftTestFiles(excludingSelf: Bool = false) throws -> [URL] {
        let testsRoot = self.repoRoot().appendingPathComponent("Tests/AgentsBarTests", isDirectory: true)
        return try self.swiftFiles(under: testsRoot).filter { file in
            !(excludingSelf && file.path.hasSuffix("Tests/AgentsBarTests/KeychainPromptSafetyAuditTests.swift"))
        }
    }
