...
@Suite(.serialized)
struct AgentsBarConfigMigratorTests {
    @Test
    func `legacy secret migration completion flag skips repeated scans`() throws {
        let suite = "AgentsBarConfigMigratorTests-skip-\(UUID().uuidString)"
        ...
        _ = AgentsBarConfigMigrator.loadOrMigrate(configStore: configStore, userDefaults: defaults, stores: stores)
    ...
    func `legacy migration completion waits for successful cleanup`() throws {
        let suite = "AgentsBarConfigMigratorTests-cleanup-failure-\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suite))
        defaults.removePersistentDomain(forName: suite)
        defer { defaults.removePersistentDomain(forName: suite) }
    ...
    func `legacy stores are kept when migrated config save fails`() throws {
        let suite = "AgentsBarConfigMigratorTests-save-failure-\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suite))
        defaults.removePersistentDomain(forName: suite)
        defer { defaults.removePersistentDomain(forName: suite) }

        let base = FileManager.default.temporaryDirectory
            .appendingPathComponent("agentsbar-tests", isDirectory: true)
    ...
    private static let legacyMigrationCompletedKey = "agentsbar.legacySecretsMigrationCompleted"

    private static func legacyStores(
        secrets: CountingLegacySecretStore,
        accountStore: CountingTokenAccountStore) -> AgentsBarConfigMigrator.LegacyStores
    {
        AgentsBarConfigMigrator.LegacyStores(
...
private final class CountingTokenAccountStore: ProviderTokenAccountStoring, @unchecked Sendable {
    ...
    func ensureFileExists() throws -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent("agentsbar-empty-accounts.json")
    }
}
