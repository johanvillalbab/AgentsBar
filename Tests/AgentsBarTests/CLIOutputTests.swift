...
struct CLIOutputTests {
    ...
    func `cli error payload is JSON array`() throws {
        let payload = AgentsBarCLI.makeCLIErrorPayload(
            message: "Nope",
            code: .failure,
            kind: .args,
            pretty: false)
    ...
    func `exit omits generic error when command already emitted payload`() {
        #expect(!AgentsBarCLI.shouldPrintExitError(code: .success, message: nil))
        #expect(!AgentsBarCLI.shouldPrintExitError(code: .failure, message: nil))
        #expect(AgentsBarCLI.shouldPrintExitError(code: .failure, message: "Nope"))
    }
