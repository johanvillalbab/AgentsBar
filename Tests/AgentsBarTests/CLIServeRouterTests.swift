...
struct CLIServeRouterTests {
    ...
    func `usage operation fingerprint separates dashboard account mode`() {
        let allAccounts = AgentsBarCLI.serveUsageOperationFingerprint(
            configFingerprint: "config",
            includeAllCodexAccounts: true)
        let selectedAccount = AgentsBarCLI.serveUsageOperationFingerprint(
            configFingerprint: "config",
            includeAllCodexAccounts: false)

        #expect(allAccounts != selectedAccount)
        #expect(allAccounts == AgentsBarCLI.serveUsageOperationFingerprint(
            configFingerprint: "config",
            includeAllCodexAccounts: true))
    }
    ...
    func `health response reports ok status and build version`() throws {
        let response = AgentsBarCLI.serveHealthResponse(version: "1.2.3")
    ...
    func `health response omits version detail when unavailable`() throws {
        let response = AgentsBarCLI.serveHealthResponse(version: nil)
        #expect(response.status == .ok)
        let object = try JSONSerialization.jsonObject(with: response.body) as? [String: Any]
        #expect(object?["status"] as? String == "ok")
    ...
    func `serve numeric options reject malformed values`() {
        #expect(AgentsBarCLI.decodeServePort(from: ParsedValues(
            positional: [],
            options: ["port": ["abc"]],
            flags: [])) == nil)
        #expect(AgentsBarCLI.decodeServePort(from: ParsedValues(
        ...
        #expect(AgentsBarCLI.decodeServeRefreshInterval(from: ParsedValues(
            positional: [],
        ...
        #expect(AgentsBarCLI.decodeServeRequestTimeout(from: ParsedValues(
            positional: [],
    ...
    func `serve help documents request timeout option`() {
        let serve = AgentsBarCLI.serveHelp(version: "0.0.0")
        let root = AgentsBarCLI.rootHelp(version: "0.0.0")

        #expect(serve.contains("--request-timeout <seconds>"))
        #expect(serve.contains("agentsbar serve --port 8080 --refresh-interval 60 --request-timeout 30"))
    ...
    func `serve config snapshot reflects provider changes`() throws {
        ...
        var firstConfig = AgentsBarConfig.makeDefault()
        firstConfig.setProviderConfig(ProviderConfig(id: .opencodego, enabled: false))
        try store.save(firstConfig)

        let firstSnapshot = try AgentsBarCLI.loadServeConfigSnapshot(configStore: store)
        ...
        let secondSnapshot = try AgentsBarCLI.loadServeConfigSnapshot(configStore: store)
        ...
        let operationKey = try AgentsBarCLI.serveOperationKey(kind: "usage", provider: nil)
        #expect(try operationKey == (AgentsBarCLI.serveOperationKey(kind: "usage", provider: nil)))
        #expect(
            AgentsBarCLI.serveCacheKey(operationKey: operationKey, configToken: firstSnapshot.cacheToken) !=
                AgentsBarCLI.serveCacheKey(operationKey: operationKey, configToken: secondSnapshot.cacheToken))
    }
    ...
    func `serve cache skips provider error payloads`() {
        ...
        #expect(AgentsBarCLI.shouldCacheServeResponse(success))
        #expect(!AgentsBarCLI.shouldCacheServeResponse(providerError))
        #expect(!AgentsBarCLI.shouldCacheServeResponse(routeError))
    }

    @Test
    func `serve provider timeout stays below the request deadline`() throws {
        let thirtySecondTimeout = try #require(AgentsBarCLI.serveProviderTimeout(requestTimeout: 30))
        let tenSecondTimeout = try #require(AgentsBarCLI.serveProviderTimeout(requestTimeout: 10))
        ...
        #expect(AgentsBarCLI.serveProviderTimeout(requestTimeout: 0) == nil)
        #expect(AgentsBarCLI.serveProviderTimeout(requestTimeout: .infinity) == nil)
        // Finite deadlines stay strictly below the request timeout at every
        // value, including sub-second ones.
        let oneSecondTimeout = try #require(AgentsBarCLI.serveProviderTimeout(requestTimeout: 1))
        let halfSecondTimeout = try #require(AgentsBarCLI.serveProviderTimeout(requestTimeout: 0.5))
        ...
        let oversizedTimeout = try #require(AgentsBarCLI.serveProviderTimeout(
    ...
    func `serve usage collection bounds a hung provider without blocking others`() async {
        ...
        let start = Date()
        let output = await AgentsBarCLI.serveCollectUsageOutputs(
            providers: providers,
            providerTimeout: 0.1)
        { provider in
            if provider == .claude {
    ...
    func `serve cache uses stable Codex account identities`() {
        ...
        let firstKey = AgentsBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: firstProjection)
        let reshapedKey = AgentsBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: reshapedProjection)
        let replacementKey = AgentsBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: replacement)
        let workspacePeerKey = AgentsBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: workspacePeer)
        let storedBeforeKey = AgentsBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: storedBeforeRefresh)
        let storedAfterKey = AgentsBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
        ...
        #expect(AgentsBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
    ...
    func `serve cache coalesces concurrent cache misses`() async {
        ...
        let responses = await withTaskGroup(of: CLILocalHTTPResponse.self) { group -> [CLILocalHTTPResponse] in
            for _ in 0..<5 {
                group.addTask {
                    await AgentsBarCLI.cachedServeResponse(
                        key: "usage:",
                        cache: cache,
                        refreshInterval: 60,
                        requestTimeout: 1)
    ...
    func `serve cache prunes expired config token entries`() async throws {
        ...
        _ = await AgentsBarCLI.cachedServeResponse(
    ...
    func `serve cache does not cache timeouts and recovers on next success`() async {
        ...
        let timeout = await AgentsBarCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 60,
            requestTimeout: 0.01)
        ...
        let success = await AgentsBarCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 60,
            requestTimeout: 1)
        ...
        let cached = await AgentsBarCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 60,
            requestTimeout: 1)
    ...
    func `serve cache serves last good payload when refresh fails`() async {
        ...
        let first = await AgentsBarCLI.cachedServeResponse(
        ...
        let failed = await AgentsBarCLI.cachedServeResponse(
            key: "usage:antigravity",
            cache: cache,
            refreshInterval: 0.05,
            requestTimeout: 1)
        ...
        let recovered = await AgentsBarCLI.cachedServeResponse(
            key: "usage:antigravity",
            cache: cache,
            refreshInterval: 0.05,
            requestTimeout: 1)
    ...
    func `cost refresh timeout serves the last good payload`() async throws {
        ...
        let timedOut = await AgentsBarCLI.cachedServeResponse(
            key: "cost:",
            cache: cache,
            refreshInterval: 0.01,
            requestTimeout: 0.01)
    ...
    func `cost refresh keeps fresh providers while replacing timed out rows`() async throws {
        ...
        let partial = await AgentsBarCLI.cachedServeResponse(
            key: "cost:",
            cache: cache,
            refreshInterval: 0.01,
            requestTimeout: 1)
    ...
    func `serve cache replaces only failed provider account rows`() async throws {
        ...
        let refreshed = await AgentsBarCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 0.05,
            requestTimeout: 1)
    ...
    func `serve stale ttl is bounded and disabled without caching`() {
        #expect(AgentsBarCLI.serveStaleTTL(refreshInterval: 0) == 0)
        #expect(AgentsBarCLI.serveStaleTTL(refreshInterval: 1) == 300)
        #expect(AgentsBarCLI.serveStaleTTL(refreshInterval: 60) == 600)
        #expect(AgentsBarCLI.serveStaleTTL(refreshInterval: 1800) == 3600)
        #expect(AgentsBarCLI.serveStaleTTL(refreshInterval: 86401) == 3600)
        #expect(AgentsBarCLI.serveStaleTTL(refreshInterval: .infinity) == 3600)
    }
    ...
    func `serve helper idle window outlives the refresh cadence`() {
        #expect(AgentsBarCLI.serveCLISessionIdleWindow(refreshInterval: 0) == 180)
        #expect(AgentsBarCLI.serveCLISessionIdleWindow(refreshInterval: 60) == 180)
        #expect(AgentsBarCLI.serveCLISessionIdleWindow(refreshInterval: 300) == 360)
    }
    ...
    func `serve request timeout zero disables the deadline`() async {
        ...
        let response = await AgentsBarCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 0,
            requestTimeout: 0)
