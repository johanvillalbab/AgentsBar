...
struct KeychainCacheStoreTests {
    ...
    func `cache ACL trusts bundled app and CLI helper`() {
        let root = URL(fileURLWithPath: "/Applications/AgentsBar.app")
        let executable = root.appendingPathComponent("Contents/MacOS/AgentsBar")
        let helper = root.appendingPathComponent("Contents/Helpers/AgentsBarCLI")
        let existing = Set([
            root.path,
            executable.path,
            helper.path,
