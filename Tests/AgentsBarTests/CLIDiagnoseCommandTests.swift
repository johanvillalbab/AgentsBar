...
struct CLIDiagnoseCommandTests {
    ...
    func `diagnose help describes generic JSON export`() {
        let help = AgentsBarCLI.diagnoseHelp(version: "0.0.0")

        #expect(help.contains("agentsbar diagnose --provider <name|all> --format json"))
        #expect(help.contains("agentsbar diagnose --provider all --format json"))
    ...
    func `diagnose output writer creates parent directories`() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("AgentsBarDiagnoseTests-\(UUID().uuidString)")
        defer { try? FileManager.default.removeItem(at: root) }

        let output = root.appendingPathComponent("nested/diagnostic.json")
        try AgentsBarCLI.writeDiagnosticExport(#"{"provider":"minimax"}"#, to: output.path)
    ...
    func `diagnose auth mode uses settings-backed MiniMax manual cookie when env token is absent`() {
        ...
        let authMode = AgentsBarCLI._resolveMiniMaxAuthModeForTesting(
            environment: [:],
            settings: settings)
    ...
    func `generic diagnose auth summary detects provider config`() {
        let summary = AgentsBarCLI._diagnosticAuthSummaryForTesting(
            provider: .openai,
            account: nil,
    ...
    func `generic diagnose auth summary requires complete Bedrock credentials`() {
        let partial = AgentsBarCLI._diagnosticAuthSummaryForTesting(
            provider: .bedrock,
            account: nil,
        ...
        let complete = AgentsBarCLI._diagnosticAuthSummaryForTesting(
            provider: .bedrock,
            account: nil,
            config: nil,
            environment: [
