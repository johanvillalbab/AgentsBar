...
struct ClaudeOAuthCredentialsStoreCLIStorageOwnershipTests {
    ...
    @Test
    func `successful agentsbar refresh is re-owned when Claude CLI storage appears`() async throws {
        let service = "com.steipete.agentsbar.cache.tests.\(UUID().uuidString)"
                    ...
                    try await ClaudeOAuthCredentialsStore.withCredentialsURLOverrideForTesting(fileURL) {
                        try await ClaudeOAuthCredentialsStore.withKeychainAccessOverrideForTesting(true) {
                            ...
                            let expiredData = self.makeCredentialsData(
                                accessToken: "expired-agentsbar-only",
                            ...
                            let refreshed = try await self.withClaudeOAuthTokenRefreshStub(handler: { request in
                                ...
                                let json = """
                                {
                                  "access_token": "fresh-agentsbar-token",
                            ...
                            #expect(refreshed.accessToken == "fresh-agentsbar-token")
                            ...
                            case let .found(entry):
                                #expect(entry.owner == .agentsbar)
                                let parsed = try ClaudeOAuthCredentials.parse(data: entry.data)
                                #expect(parsed.accessToken == "fresh-agentsbar-token")
                                #expect(parsed.refreshToken == "fresh-refresh-token")
                            default:
                                Issue.record("Expected refreshed AgentsBar-owned cache entry")
                            }
                            ...
                            #expect(recordAfterCLIStorageAppears.credentials.accessToken == "fresh-agentsbar-token")
    ...
    @Test
    func `load record treats agentsbar cache as claude CLI owned when credentials file exists`() throws {
        let service = "com.steipete.agentsbar.cache.tests.\(UUID().uuidString)"
                ...
                try ClaudeOAuthCredentialsStore.withIsolatedMemoryCacheForTesting {
                    try ClaudeOAuthCredentialsStore.withCredentialsURLOverrideForTesting(fileURL) {
                        try ClaudeOAuthCredentialsStore.withKeychainAccessOverrideForTesting(true) {
                            ...
                            #expect(record.credentials.accessToken == "agentsbar-cache")
    ...
    @Test
    func `load with auto refresh delegates expired agentsbar cache when credentials file exists`() async throws {
        let service = "com.steipete.agentsbar.cache.tests.\(UUID().uuidString)"
        try await KeychainCacheStore.withServiceOverrideForTesting(service) {
            KeychainCacheStore.setTestStoreForTesting(true)
            defer { KeychainCacheStore.setTestStoreForTesting(false) }
                ...
                try await ClaudeOAuthCredentialsStore.withIsolatedMemoryCacheForTesting {
                    try await ClaudeOAuthCredentialsStore.withCredentialsURLOverrideForTesting(fileURL) {
                        try await ClaudeOAuthCredentialsStore.withKeychainAccessOverrideForTesting(true) {
                            ...
                            let expiredData = self.makeCredentialsData(
                                accessToken: "expired-agentsbar-with-file",
                                expiresAt: Date(timeIntervalSinceNow: -3600),
                                refreshToken: "cached-refresh-token")
                            KeychainCacheStore.store(
                                key: cacheKey,
    ...
    @Test
    func `load with auto refresh keeps agentsbar cache ownership without Claude CLI storage`() async throws {
        let service = "com.steipete.agentsbar.cache.tests.\(UUID().uuidString)"
        try await KeychainCacheStore.withServiceOverrideForTesting(service) {
            KeychainCacheStore.setTestStoreForTesting(true)
            defer { KeychainCacheStore.setTestStoreForTesting(false) }
                            ...
                            await ClaudeOAuthRefreshFailureGate.$shouldAttemptOverride.withValue(false) {
                                do {
                                    _ = try await ClaudeOAuthCredentialsStore.loadWithAutoRefresh(
                                        ...
                                        allowKeychainPrompt: false,
                                        respectKeychainPromptCooldown: true)
                                    Issue.record("Expected direct AgentsBar refresh failure")
                                } catch let error as ClaudeOAuthCredentialsError {
    ...
    @Test
    func `load record treats agentsbar cache as claude CLI owned when Claude keychain item exists`() throws {
        let service = "com.steipete.agentsbar.cache.tests.\(UUID().uuidString)"
        try KeychainCacheStore.withServiceOverrideForTesting(service) {
            KeychainCacheStore.setTestStoreForTesting(true)
            defer { KeychainCacheStore.setTestStoreForTesting(false) }
    ...
    @Test
    func `load record ignores agentsbar cache in never prompt mode`() throws {
        let service = "com.steipete.agentsbar.cache.tests.\(UUID().uuidString)"
        try KeychainCacheStore.withServiceOverrideForTesting(service) {
            KeychainCacheStore.setTestStoreForTesting(true)
            defer { KeychainCacheStore.setTestStoreForTesting(false) }
