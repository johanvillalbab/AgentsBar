...
struct CLICostTests {
    ...
    func `cost json shortcut does not enable json logs`() throws {
        let signature = AgentsBarCLI._costSignatureForTesting()
    ...
    func `renders cost text snapshot`() {
        ...
        let output = AgentsBarCLI.renderCostText(provider: .claude, snapshot: snap, useColor: false)
    ...
    func `renders codex project grouped cost text`() {
        ...
        let output = AgentsBarCLI.renderCostText(
            provider: .codex,
            snapshot: snap,
            groupBy: .project,
            useColor: false)
    ...
    func `codex cost payload includes project rollups`() throws {
        let snapshot = CostUsageTokenSnapshot(
            ...
            updatedAt: Date(timeIntervalSince1970: 1_700_000_000))
        let payload = AgentsBarCLI.makeCostPayload(provider: .codex, snapshot: snapshot, error: nil)
    ...
    func `cursor cookie source off produces a failed JSON payload`() throws {
        ...
        let error = try #require(AgentsBarCLI.cursorCostAvailabilityError(.cursor, settings: settings))
        let payload = AgentsBarCLI.makeCostPayload(provider: .cursor, snapshot: nil, error: error)
        let json = try #require(AgentsBarCLI.encodeJSON([payload], pretty: false))

        #expect(AgentsBarCLI.mapError(error) == .failure)
        #expect(json.contains("\"provider\":\"cursor\""))
        #expect(json.contains("\"code\":1"))
        #expect(json.contains("cookie source is set to Off"))
        #expect(AgentsBarCLI.cursorCostAvailabilityError(.cursor, settings: nil) == nil)
        #expect(AgentsBarCLI.cursorCostAvailabilityError(.codex, settings: settings) == nil)
    }
    ...
    func `cursor manual cookie source rejects an empty header`() throws {
        ...
        #expect(AgentsBarCLI.cursorCostHeaderOverride(.cursor, settings: settings) == nil)
    }

    @Test
    func `cursor settings resolution errors fail closed`() throws {
        let resolutionError = CursorCostSettingsTestError()
        let error = try #require(AgentsBarCLI.cursorCostAvailabilityError(
            .cursor,
            settings: nil,
            resolutionError: resolutionError))

        #expect(error.localizedDescription == resolutionError.localizedDescription)
        #expect(AgentsBarCLI.cursorCostAvailabilityError(
            .codex,
            settings: nil,
