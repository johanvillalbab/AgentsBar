...
struct CLISnapshotTests {
    ...
    func `defaults to usage when no command provided`() {
        #expect(AgentsBarCLI.effectiveArgv([]) == ["usage"])
        #expect(AgentsBarCLI.effectiveArgv(["--format", "json"]).first == "usage")
        #expect(AgentsBarCLI.effectiveArgv(["usage", "--format", "json"]).first == "usage")
    }
