...
struct MenuBarLayoutEditorTests {
    ...
    func `palette drag transfer representation round trips`() async throws {
        ...
        #expect(MenuBarLayoutDragItem.exportedContentTypes() == [.agentsBarMenuLayoutItem])
        #expect(MenuBarLayoutDragItem.importedContentTypes() == [.agentsBarMenuLayoutItem])

        let data = try await payload.exported(as: .agentsBarMenuLayoutItem)
        let decoded = try await MenuBarLayoutDragItem(
            importing: data,
            contentType: .agentsBarMenuLayoutItem)
