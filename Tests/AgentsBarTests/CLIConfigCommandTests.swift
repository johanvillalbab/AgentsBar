...
struct CLIConfigCommandTests {
    ...
    func `config set api key parses provider stdin and no enable flags`() throws {
        let parser = CommandParser(signature: AgentsBarCLI._configSetAPIKeySignatureForTesting())
    ...
    func `config set api key stores key and enables provider`() {
        ...
        let updated = AgentsBarCLI.configSettingAPIKey(
            config,
            provider: .elevenlabs,
    ...
    func `config set api key stores zai team token account`() throws {
        let config = AgentsBarConfig.makeDefault()
        let options = try AgentsBarCLI.resolveConfigAPIKeyAccountOptions(
            provider: .zai,
            label: "Team",
            usageScope: "team",
    ...
    func `config set api key rejects incomplete zai team account options`() {
        #expect(throws: CLIArgumentError.self) {
            _ = try AgentsBarCLI.resolveConfigAPIKeyAccountOptions(
                provider: .zai,
                label: "Team",
                usageScope: "team",
                organizationID: "org-team",
    ...
    func `config provider toggle parses provider and json flags`() throws {
        let parser = CommandParser(signature: AgentsBarCLI._configProviderToggleSignatureForTesting())
        let parsed = try parser.parse(arguments: [
            "--provider", "grok",
            "--json",
            "--pretty",
    ...
    func `config provider toggle enables and disables provider`() {
        let config = AgentsBarConfig.makeDefault()
        let enabled = AgentsBarCLI.configSettingProviderEnabled(config, provider: .grok, enabled: true)
        let disabled = AgentsBarCLI.configSettingProviderEnabled(enabled, provider: .grok, enabled: false)
    ...
    func `config provider status includes effective default`() throws {
        ...
        let statuses = AgentsBarCLI.configProviderStatuses(config)
    ...
    func `config set api key rejects ambiguous input`() {
        #expect(throws: CLIArgumentError.self) {
            try AgentsBarCLI.resolveConfigAPIKeyInput(apiKey: "xi-test-token", readFromStdin: true)
        }
    }

    @Test
    func `config help documents set api key`() {
        let help = AgentsBarCLI.configHelp(version: "0.0.0")
