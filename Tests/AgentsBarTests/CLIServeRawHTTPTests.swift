...
/// Raw-socket coverage for `agentsbar serve`: boots the real `CLILocalHTTPServer` on an
...
struct CLIServeRawHTTPTests {
    ...
    func `snapshot response preserves usage cache metadata`() async throws {
        ...
        try store.save(AgentsBarConfig(providers: UsageProvider.allCases.map {
        ...
        let response = await AgentsBarCLI.handleServeRequest(request, runtime: runtime)
        ...
        body: (UInt16) async throws -> Void) async throws
    {
        ...
        try await Self.withServer(
            handler: { request in
                await AgentsBarCLI.handleServeRequest(request, runtime: runtime)
            },
            body: body)
    }
