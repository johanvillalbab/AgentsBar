...
struct ConfigValidationTests {
    ...
    func `reports unsafe hook rule fields`() {
        ...
        let config = AgentsBarConfig(
            providers: [ProviderConfig(id: .codex)],
            hooks: HooksConfig(enabled: true, events: invalidRules))
        let codes = Set(AgentsBarConfigValidator.validate(config).map(\.code))
    ...
    func `reports hook workload limits`() {
        ...
        let config = AgentsBarConfig(providers: [], hooks: HooksConfig(enabled: true, events: rules))
        let codes = Set(AgentsBarConfigValidator.validate(config).map(\.code))
    ...
    func `fresh config defaults Alibaba Token Plan to International`() throws {
        ...
        let issues = AgentsBarConfigValidator.validate(config)
    ...
    func `normalization preserves legacy Alibaba Token Plan region`() throws {
        let config = AgentsBarConfig(providers: [
    ...
    func `reports invalid Alibaba Token Plan region`() {
        var config = AgentsBarConfig.makeDefault()
    ...
    func `sub2api accepts HTTPS and loopback HTTP base URLs`() {
        for host in ["https://sub2api.example.com", "http://127.0.0.1:8080"] {
            ...
            config.setProviderConfig(ProviderConfig(
                ...
                apiKey: "fixture",
                enterpriseHost: host))
            let invalidHostIssue = AgentsBarConfigValidator.validate(config).first { issue in
    ...
    func `unsupported enterprise host warning lists every supported provider`() throws {
        ...
        let issue = try #require(AgentsBarConfigValidator.validate(config).first(where: {
    ...
    @Test
    func `config store default url honors legacy environment override`() {
        let url = AgentsBarConfigStore.defaultURL(environment: [
            AgentsBarConfigStore.legacyPathEnvironmentKey: "~/tmp/codexbar-test-config.json",
        ])
    ...
    @Test
    func `config store default url prefers agentsbar config env over legacy env`() {
        let url = AgentsBarConfigStore.defaultURL(environment: [
            AgentsBarConfigStore.pathEnvironmentKey: "~/tmp/agentsbar-test-config.json",
            AgentsBarConfigStore.legacyPathEnvironmentKey: "~/tmp/codexbar-test-config.json",
        ])

        #expect(url.path.hasSuffix("/tmp/agentsbar-test-config.json"))
    }
    ...
    func `config store default url honors xdg config home`() throws {
        ...
        let url = AgentsBarConfigStore.defaultURL(
            home: home,
            environment: [
                AgentsBarConfigStore.xdgConfigHomeEnvironmentKey: xdgHome.path,
            ],
    ...
    func `config store default url ignores relative xdg config home`() throws {
        ...
        let url = AgentsBarConfigStore.defaultURL(
            home: home,
            environment: [
                AgentsBarConfigStore.xdgConfigHomeEnvironmentKey: "relative-config",
            ],
            fileManager: fileManager)
    ...
    func `config store default url creates in xdg default for new installs`() throws {
        ...
        let url = AgentsBarConfigStore.defaultURL(home: home, environment: [:], fileManager: fileManager)

        #expect(url == Self.configURL(in: home.appendingPathComponent(".config", isDirectory: true)))
    }

    @Test
    func `config store default url keeps existing legacy dot codexbar config`() throws {
        let fileManager = FileManager.default
        let home = try Self.makeTemporaryHome()
        defer { try? fileManager.removeItem(at: home) }
        let legacy = Self.legacyConfigURL(in: home)
    ...
    @Test
    func `config store default url prefers existing xdg codexbar over dot codexbar`() throws {
        let fileManager = FileManager.default
        let home = try Self.makeTemporaryHome()
        defer { try? fileManager.removeItem(at: home) }
        let xdgCodexbar = Self.legacyXDGConfigURL(in: home.appendingPathComponent(".config", isDirectory: true))
        let legacy = Self.legacyConfigURL(in: home)
        try Self.touch(legacy, fileManager: fileManager)
        try Self.touch(xdgCodexbar, fileManager: fileManager)

        let url = AgentsBarConfigStore.defaultURL(home: home, environment: [:], fileManager: fileManager)

        #expect(url == xdgCodexbar)
    }
    ...
    private static func makeTemporaryHome() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("AgentsBarConfigStoreTests-\(UUID().uuidString)", isDirectory: true)
    ...
    private static func legacyXDGConfigURL(in directory: URL) -> URL {
        directory
            .appendingPathComponent("codexbar", isDirectory: true)
            .appendingPathComponent("config.json")
    }
