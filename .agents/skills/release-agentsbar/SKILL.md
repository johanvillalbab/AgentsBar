---
name: release-agentsbar
description: "AgentsBar release: versioning, notarization, appcast, Homebrew, post-release bump."
---
...
- repo: `~/Projects/agentsbar`
- release script: `Scripts/release.sh`
- signing/notarization: `Scripts/sign-and-notarize.sh`
- appcast: `Scripts/make_appcast.sh`, `appcast.xml`
- release assets: `AgentsBar-macos-universal-<version>.zip`, `AgentsBar-macos-universal-<version>.dSYM.zip`
- packaged app: `AgentsBar.app`
- version file: `version.env`
- changelog: `CHANGELOG.md`
- Homebrew tap: `~/Projects/homebrew-tap`
- cask: `~/Projects/homebrew-tap/Casks/agentsbar.rb`
- formula: `~/Projects/homebrew-tap/Formula/agentsbar.rb`
- CLI release workflow: `.github/workflows/release-cli.yml`

Normal release:

```bash
tmux new-session -d -s agentsbar-release 'op run --account my.1password.com --env-file /tmp/agentsbar-release-op.env -- Scripts/release.sh'
tmux attach -t agentsbar-release
```
...
If widget metadata generation times out, `AGENTSBAR_WIDGET_METADATA_TIMEOUT_SECONDS=600` is a known-good floor.

AgentsBar CLI tarballs are not produced by `Scripts/release.sh` itself. The GitHub release event triggers `.github/workflows/release-cli.yml`, which builds and uploads:

- `AgentsBarCLI-v<version>-macos-arm64.tar.gz`
- `AgentsBarCLI-v<version>-macos-x86_64.tar.gz`
- `AgentsBarCLI-v<version>-linux-aarch64.tar.gz`
- `AgentsBarCLI-v<version>-linux-x86_64.tar.gz`
- matching `.sha256` files

If the workflow fails only in `update-homebrew-tap` with GitHub API rate limiting, the CLI assets may already be uploaded. Verify assets live, then update `Formula/agentsbar.rb` manually from the tarball checksums.
...
PY
codesign --verify --deep --strict --verbose=2 AgentsBar.app
spctl --assess --type execute --verbose AgentsBar.app
```

For Homebrew:

```bash
shasum -a 256 AgentsBar-macos-universal-<VERSION>.zip
cd /Users/steipete/Projects/homebrew-tap
python3 .github/scripts/update_formula.py --formula agentsbar --tag v<VERSION> --repository johanvillalbab/agentsbar --artifact-template 'AgentsBarCLI-{tag}-{target}.tar.gz' --target-aliases 'darwin_arm64=macos-arm64,darwin_amd64=macos-x86_64,linux_arm64=linux-aarch64,linux_amd64=linux-x86_64'
brew fetch --cask --force --retry agentsbar
brew fetch --formula --force --retry steipete/tap/agentsbar
```
...
open -n AgentsBar.app
/usr/libexec/PlistBuddy -c 'Print :CFBundleShortVersionString' AgentsBar.app/Contents/Info.plist
/usr/libexec/PlistBuddy -c 'Print :CFBundleVersion' AgentsBar.app/Contents/Info.plist
```
