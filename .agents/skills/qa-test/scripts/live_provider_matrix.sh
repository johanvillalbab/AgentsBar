...
WEB_TIMEOUT="${AGENTSBAR_QA_WEB_TIMEOUT:-12}"
CASE_TIMEOUT="${AGENTSBAR_QA_CASE_TIMEOUT:-60}"
...
Environment:
  AGENTSBAR_CLI=/path/to/AgentsBarCLI
  AGENTSBAR_CONFIG=/path/to/config.json
  AGENTSBAR_QA_WEB_TIMEOUT=12
  AGENTSBAR_QA_CASE_TIMEOUT=60
USAGE
}

if [[ ! -x "$CLI" ]]; then
  echo "missing AgentsBarCLI at $CLI" >&2
  exit 2
fi
...
case "$mode" in
  --enabled)
    ...
    if ! "$CLI" config providers --format json --json-only >"$provider_status" 2>"$provider_err"; then
      ...
      echo "failed to list providers via AgentsBarCLI config providers" >&2
      exit 2
    fi
...
NODE
      rm -f "$provider_status" "$provider_err" "$provider_list"
      echo "failed to parse AgentsBarCLI config providers output" >&2
      exit 2
    fi
    ...
    if [[ "${#providers[@]}" -eq 0 ]]; then
      echo "no enabled providers found via AgentsBarCLI config providers" >&2
      exit 2
    fi
    ;;
  --default)
