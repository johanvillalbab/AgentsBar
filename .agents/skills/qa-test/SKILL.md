---
name: qa-test
description: "AgentsBar live QA/e2e testing: run provider usage matrix checks, validate real app config, use Peekaboo for menu proof, use Browser Use/official docs for API spec or logged-in dashboard checks, and handle 1Password credentials safely."
---
...
- Work from the AgentsBar repo checkout.
- Use the packaged CLI first: `AgentsBar.app/Contents/Helpers/AgentsBarCLI`.
- Do not use `AgentsBar.app/Contents/MacOS/agentsbar`; that is the app binary and may appear to hang as a CLI.
...
- `--enabled` asks `AgentsBarCLI config providers` for enabled providers, honoring `AGENTSBAR_CONFIG` and default toggles.
...
```bash
AgentsBar.app/Contents/Helpers/AgentsBarCLI config validate
stat -f '%Lp %N' "$HOME/.config/agentsbar/config.json"
```
...
jq '(.providers // []) |= map(.apiKey = (if .apiKey then "<redacted>" else .apiKey end) |
  ...
  "$HOME/.config/agentsbar/config.json"
```

Before editing config, make a backup:

```bash
cp "$HOME/.config/agentsbar/config.json" "$HOME/.config/agentsbar/config.pre-qa-$(date +%Y%m%d%H%M%S).json"
chmod 600 "$HOME/.config/agentsbar"/config.pre-qa-*.json
```
...
```bash
pkill -x AgentsBar || pkill -f 'AgentsBar.app/Contents/MacOS/AgentsBar' || true
open -n "$PWD/AgentsBar.app"
peekaboo menu list-all --json | rg -i 'agentsbar'
peekaboo menu click-extra --title agentsbar-merged --json
screencapture -x /tmp/agentsbar-live-menu.png
```

Crop top-right menu if needed:

```bash
sips --cropToHeightWidth 900 340 --cropOffset 20 2650 /tmp/agentsbar-live-menu.png \
  --out /tmp/agentsbar-live-menu-crop.png >/dev/null
```
