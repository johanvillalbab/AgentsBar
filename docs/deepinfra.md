...
DeepInfra is API-only. AgentsBar uses DeepInfra's documented billing endpoints and does not send model prompts or account data to any other service.

## Setup

1. Create an API key in the [DeepInfra dashboard](https://deepinfra.com/dash).
2. In AgentsBar, open **Settings > Providers > DeepInfra > API tokens** and add the key.

You can instead set `DEEPINFRA_API_KEY` or `DEEPINFRA_TOKEN` in AgentsBar's environment. `DEEPINFRA_API_KEY` takes precedence when both are present.

## Data source

AgentsBar sends the key as a bearer token to:
...
DeepInfra represents prepaid funds as a negative `stripe_balance`; AgentsBar converts that to a positive “available” amount. A positive value is shown as money owed.
...
- If the account has a positive spending limit, AgentsBar shows billing-cycle spend against that limit.
...
AgentsBar never logs the API key or raw billing response.
