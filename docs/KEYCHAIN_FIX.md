...
The original fix (migrating legacy AgentsBar keychain items to `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly`) is
still in place, but the architecture has changed:

- Provider settings and manual secrets are now persisted in `~/.config/agentsbar/config.json`.
- Legacy keychain stores are still present mainly to migrate old installs, then clear old items.
- Keychain is still used for runtime cache entries (for example `com.steipete.agentsbar.cache`) and Claude OAuth
...
| --- | --- |
| AgentsBar stores provider credentials only in keychain | Manual/provider settings are config-file backed (`~/.config/agentsbar/config.json`), while keychain is still used for runtime caches and Claude OAuth bootstrap fallback. |
| `ClaudeOAuthCredentials.swift` migrated AgentsBar-owned Claude OAuth keychain items | Claude OAuth primary source is Claude CLI keychain service (`Claude Code-credentials`), with AgentsBar cache in `com.steipete.agentsbar.cache` (`oauth.claude`). |
| Migration runs in `AgentsBarApp.init()` | Migration runs in `HiddenWindowView` `.task` via detached task (`KeychainMigration.migrateIfNeeded()`). |
...
### 1. Legacy AgentsBar keychain migration (V1)
`Sources/AgentsBar/KeychainMigration.swift` migrates legacy `com.steipete.AgentsBar` items (for example
`claude-cookie`) to `AfterFirstUnlockThisDeviceOnly`.

- Gate key: `KeychainMigrationV1Completed`
- Runs once unless flag is reset.
- Covers legacy AgentsBar-managed accounts only (not Claude CLI's own keychain service).

### 2. Claude OAuth bootstrap path
`Sources/AgentsBarCore/Providers/Claude/ClaudeOAuth/ClaudeOAuthCredentials.swift`

Load order for credentials:
1. Environment override (`AGENTSBAR_CLAUDE_OAUTH_TOKEN`, scopes env key).
2. In-memory cache.
3. AgentsBar keychain cache (`com.steipete.agentsbar.cache`, account `oauth.claude`).
...
When AgentsBar does not have usable OAuth credentials in its own cache (`com.steipete.agentsbar.cache` / `oauth.claude`),
...
`Sources/AgentsBarCore/CookieHeaderCache.swift` and `Sources/AgentsBarCore/KeychainCacheStore.swift`

- Browser-imported Claude session cookies are cached in keychain service `com.steipete.agentsbar.cache`.
...
- Users can clear browser-cookie cache entries from **Preferences → Debug → Caches** or with
  `agentsbar cache clear --cookies`. `--provider <id>` scopes cookie clearing to one provider and includes scoped
...
- Keychain cache store (`com.steipete.agentsbar.cache`) also writes with `ThisDeviceOnly`.
...
```bash
defaults read com.steipete.agentsbar KeychainMigrationV1Completed
```

### Check Claude OAuth keychain cooldown
```bash
defaults read com.steipete.agentsbar claudeOAuthKeychainDeniedUntil
```

### Inspect keychain-related logs
```bash
log show --predicate 'subsystem == "com.steipete.agentsbar" && (category == "keychain-migration" || category == "keychain-preflight" || category == "keychain-prompt" || category == "keychain-cache" || category == "claude-usage" || category == "cookie-cache")' --last 10m
```

### Reset migration for local testing
```bash
defaults delete com.steipete.agentsbar KeychainMigrationV1Completed
./Scripts/compile_and_run.sh
```

## Key files (current)

- `Sources/AgentsBar/KeychainMigration.swift`
- `Sources/AgentsBar/HiddenWindowView.swift`
- `Sources/AgentsBarCore/Providers/Claude/ClaudeOAuth/ClaudeOAuthCredentials.swift`
- `Sources/AgentsBarCore/Providers/Claude/ClaudeOAuth/ClaudeOAuthKeychainAccessGate.swift`
- `Sources/AgentsBarCore/KeychainAccessPreflight.swift`
- `Sources/AgentsBarCore/KeychainNoUIQuery.swift`
- `Sources/AgentsBarCore/KeychainCacheStore.swift`
- `Sources/AgentsBarCore/CookieHeaderCache.swift`
