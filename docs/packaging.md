...
## Scripts
- `Scripts/package_app.sh`: builds host arch with ad-hoc signing by default; set `ARCHES="arm64 x86_64"` for universal. Verifies slices. Stable-certificate packaging requires explicit `AGENTSBAR_SIGNING=identity` plus `APP_IDENTITY`.
...
## Bundle contents
- `AgentsBarWidget.appex` is built by `WidgetExtension/AgentsBarWidgetExtension.xcodeproj` as a real macOS app extension, then bundled with app-group entitlements.
- `AgentsBarCLI` copied to `AgentsBar.app/Contents/Helpers/` for symlinking.
