...
AgentsBar's OpenAI API provider targets the API Platform organization dashboard, not ChatGPT/Codex subscription limits.
...
Settings → Providers → OpenAI writes the same `~/.config/agentsbar/config.json` field. `OPENAI_ADMIN_KEY` is preferred over
...
keys, so they cannot read the organization usage and cost endpoints used by AgentsBar. Configure an organization Admin
API key instead; AgentsBar reports this distinction when a project or service-account key is rejected.
