...
read_when:
  ...
  - Troubleshooting LiteLLM API-key usage in AgentsBar
---
...
Configure it in Settings -> Providers -> LiteLLM, or in `~/.config/agentsbar/config.json`:
...
`LITELLM_BASE_URL` may include `/v1`; AgentsBar strips that suffix before calling LiteLLM management endpoints.
...
All requests use `Authorization: Bearer <apiKey>`. AgentsBar does not request or store a LiteLLM master key.
...
endpoint. AgentsBar validates returned user and team IDs against `/key/info` before displaying usage.

## Security

Treat LiteLLM keys as secrets. AgentsBar stores configured keys only in provider config or token-account storage and
