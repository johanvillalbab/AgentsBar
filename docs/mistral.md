...
AgentsBar reads Mistral billing usage with the Mistral web session from `admin.mistral.ai`. It can also fetch credit
...
AgentsBar requests the current UTC month from Mistral Admin:
...
When a CSRF token is available, AgentsBar also makes a bounded best-effort request for Mistral Vibe plan usage:

- `GET https://console.mistral.ai/api-ui/trpc/billing.vibeUsage?...`

For the console request, AgentsBar forwards only the `csrftoken` and `ory_session_*` cookies. Other
...
```bash
agentsbar usage --provider mistral --verbose
```
...
does not expose data for the account, AgentsBar keeps the main Mistral usage result.

## Related Files

- `Sources/AgentsBarCore/Providers/Mistral/MistralProviderDescriptor.swift`
- `Sources/AgentsBarCore/Providers/Mistral/MistralUsageFetcher.swift`
- `Sources/AgentsBarCore/Providers/Mistral/MistralModels.swift`
- `Sources/AgentsBarCore/Providers/Mistral/MistralCookieImporter.swift`
- `Sources/AgentsBar/Providers/Mistral/MistralProviderImplementation.swift`
