---
summary: "AgentsBar config file layout for CLI + app settings."
read_when:
  - "Editing the AgentsBar config file or moving settings off Keychain."
...
AgentsBar reads a single JSON config file for CLI and app provider settings.
...
## Location
- `AGENTSBAR_CONFIG=/path/to/config.json` when set.
- `$XDG_CONFIG_HOME/agentsbar/config.json` when `XDG_CONFIG_HOME` is set to an absolute path. Relative values are
  ignored.
- `~/.config/agentsbar/config.json` by default for new installs.
- Legacy CodexBar installs: `~/.codexbar/config.json` or `~/.config/codexbar/config.json` still load when no XDG config exists.
- The directory is created if missing.
- Permissions are set to `0600` whenever AgentsBar writes the file on macOS and Linux.
...
event's `AGENTSBAR_*` variables; AgentsBar provider keys and tokens are not inherited. The same event is also encoded as
...
- `refresh_failed`: a provider refresh fails; `AGENTSBAR_STATUS` is a coarse category such as `timeout`, `offline`,
...
Payload environment variables are `AGENTSBAR_EVENT`, `AGENTSBAR_PROVIDER`, `AGENTSBAR_TIMESTAMP`, and, when available,
`AGENTSBAR_ACCOUNT`, `AGENTSBAR_WINDOW`, `AGENTSBAR_USAGE_PERCENT`, `AGENTSBAR_USED`, `AGENTSBAR_LIMIT`,
`AGENTSBAR_RESET_AT`, and `AGENTSBAR_STATUS`. Enabling Hide personal info omits `AGENTSBAR_ACCOUNT` and the matching JSON
field.

The stdin JSON uses the same camel-case field names without the `AGENTSBAR_` prefix. Dates are UTC ISO 8601 strings,
...
The v1 field names and meanings are compatibility-stable. Hook consumers should ignore unknown fields so AgentsBar can
...
`agentsbar`, and a cookie written by tooling is shown in the app after reload.
...
```bash
agentsbar config validate
agentsbar usage --provider example-provider --verbose
```
...
printf '%s' "$GROQ_API_KEY" | agentsbar config set-api-key --provider groq --stdin
printf '%s' "$LLM_PROXY_API_KEY" | agentsbar config set-api-key --provider llmproxy --stdin
printf '%s' "$LITELLM_API_KEY" | agentsbar config set-api-key --provider litellm --stdin
printf '%s' "$CLAWROUTER_API_KEY" | agentsbar config set-api-key --provider clawrouter --stdin
printf '%s' "$SUB2API_API_KEY" | agentsbar config set-api-key --provider sub2api --stdin
printf '%s' "$AIAND_API_KEY" | agentsbar config set-api-key --provider aiand --stdin
```
...
Manual cookies are secrets. Keep the AgentsBar config file private, leave its permissions at `0600`, never commit it,
...
## Provider IDs
Current IDs (see `Sources/AgentsBarCore/Providers/Providers.swift`):
...
- Validate the file with `agentsbar config validate` (JSON output available with `--format json`).
