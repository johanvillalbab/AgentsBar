...
**Files:**
- Create: `Sources/AgentsBarCore/Providers/Kilo/KiloOrganization.swift`
- Create: `Tests/AgentsBarTests/KiloOrganizationTests.swift`
...
```bash
git add Sources/AgentsBarCore/Providers/Kilo/KiloOrganization.swift \
        Tests/AgentsBarTests/KiloOrganizationTests.swift
...
**Files:**
- Create: `Sources/AgentsBarCore/Providers/Kilo/KiloUsageScope.swift`
- Modify: `Tests/AgentsBarTests/KiloOrganizationTests.swift`

- [ ] **Step 2.1: Append failing tests**

Append to `Tests/AgentsBarTests/KiloOrganizationTests.swift`:
...
```bash
git add Sources/AgentsBarCore/Providers/Kilo/KiloUsageScope.swift \
        Tests/AgentsBarTests/KiloOrganizationTests.swift
...
**Files:**
- Modify: `Sources/AgentsBarCore/Providers/Kilo/KiloUsageFetcher.swift`
- Modify: `Tests/AgentsBarTests/KiloUsageFetcherTests.swift`

- [ ] **Step 3.1: Append failing test for header injection**

Append the following inside `struct KiloUsageFetcherTests` in `Tests/AgentsBarTests/KiloUsageFetcherTests.swift`:
...
In `Sources/AgentsBarCore/Providers/Kilo/KiloUsageFetcher.swift`:
...
```bash
git add Sources/AgentsBarCore/Providers/Kilo/KiloUsageFetcher.swift \
        Tests/AgentsBarTests/KiloUsageFetcherTests.swift
...
Append to `Sources/AgentsBarCore/Providers/Kilo/KiloUsageFetcher.swift` (inside the `KiloUsageFetcher` struct):
...
**Files:**
- Modify: `Sources/AgentsBarCore/Config/AgentsBarConfig.swift`
- Tests: covered indirectly via Task 6 SettingsStore tests.

- [ ] **Step 5.1: Add fields to `ProviderConfig`**

In `Sources/AgentsBarCore/Config/AgentsBarConfig.swift`, inside `public struct ProviderConfig`:
...
```bash
git add Sources/AgentsBarCore/Config/AgentsBarConfig.swift
...
**Files:**
- Modify: `Sources/AgentsBar/Providers/Kilo/KiloSettingsStore.swift`
- Create: `Tests/AgentsBarTests/KiloSettingsStoreTests.swift`
...
Append to `Sources/AgentsBar/Providers/Kilo/KiloSettingsStore.swift`:
...
```bash
git add Sources/AgentsBar/Providers/Kilo/KiloSettingsStore.swift \
        Tests/AgentsBarTests/KiloSettingsStoreTests.swift
...
**Files:**
- Modify: `Sources/AgentsBarCore/Providers/Kilo/KiloProviderDescriptor.swift`
- Modify: `Sources/AgentsBar/UsageStore+Refresh.swift`
- Create: `Sources/AgentsBar/Providers/Kilo/UsageStore+KiloOrgRefresh.swift`
...
In `Sources/AgentsBar/UsageStore.swift`, find the closest place where Codex-related published properties live (e.g. near `codexAccountSnapshots`) and add:
...
In `Sources/AgentsBar/UsageStore+Refresh.swift`, find the existing `tokenAccounts` fan-out block in `refreshProvider`:
...
```bash
git add Sources/AgentsBar/UsageStore.swift \
        Sources/AgentsBar/UsageStore+Refresh.swift \
        Sources/AgentsBar/Providers/Kilo/UsageStore+KiloOrgRefresh.swift
...
**Files:**
- Modify: `Sources/AgentsBar/StatusItemController+MenuCardModel.swift` (or the file that produces Kilo menu rows)
- Modify: `Sources/AgentsBar/MenuDescriptor.swift` if needed
...
```bash
grep -n "case \\.kilo" Sources/AgentsBar/StatusItemController*.swift Sources/AgentsBar/Menu*.swift 2>&1 | head -15
```
...
```bash
git add Sources/AgentsBar/StatusItemController+MenuCardModel.swift \
        Sources/AgentsBar/MenuDescriptor.swift
...
**Files:**
- Modify: `Sources/AgentsBar/Providers/Kilo/KiloProviderImplementation.swift`
- Possibly modify: `Sources/AgentsBar/PreferencesProviderDetailView.swift` and `Sources/AgentsBarCore/Providers/ProviderDescriptor.swift` if a new descriptor variant is needed.
...
grep -n "enum Kind\|case info\|case toggleList\|case checkboxList" \
    Sources/AgentsBarCore/Providers/ProviderDescriptor.swift \
    Sources/AgentsBar/PreferencesProviderDetailView.swift 2>&1 | head -20
```

If a toggle-list kind exists, reuse it. Otherwise, add a new `ProviderSettingsOrganizationsDescriptor` in `Sources/AgentsBarCore/Providers/ProviderDescriptor.swift`:
...
In `Sources/AgentsBar/Providers/Kilo/KiloProviderImplementation.swift`:
...
In `Sources/AgentsBar/PreferencesProviderDetailView.swift`, follow the existing pattern used by `settingsTokenAccounts`. Add a stored property `settingsOrganizations: ProviderSettingsOrganizationsDescriptor?`, source it from the provider implementation, and render it as a SwiftUI section under the API key:
...
```bash
git add Sources/AgentsBarCore/Providers/ProviderDescriptor.swift \
        Sources/AgentsBar/Providers/Kilo/KiloProviderImplementation.swift \
        Sources/AgentsBar/PreferencesProviderDetailView.swift
...
AgentsBar can show usage for any Kilo organization the API key belongs to.
...
- When at least one organization is enabled, the menu renders one Kilo card per
  enabled scope.
- The AgentsBar fetcher sends the standard `X-KILOCODE-ORGANIZATIONID` header on
...
```bash
gh repo view noefabris/AgentsBar --json url 2>&1 | head -5
```

If 404, fork:

```bash
gh repo fork johanvillalbab/agentsbar --remote=false --clone=false
```

- [ ] **Step 12.2: Add fork as a remote (if missing) and push**

```bash
git remote get-url fork 2>/dev/null || git remote add fork https://github.com/noefabris/AgentsBar.git
