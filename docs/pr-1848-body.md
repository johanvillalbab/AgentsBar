...
Fixes the background browser-launch regression in https://github.com/johanvillalbab/agentsbar/issues/1844: when Claude Code stores only MCP OAuth state in `Claude Code-credentials` (no `claudeAiOauth`), AgentsBar no longer runs background delegated `claude /status` refresh—which can launch the default browser via `/usr/bin/open`.

**Scope:** fail-closed safety guard for both keychain readers. Discovery of Claude Code 2.1.x's primary OAuth storage location remains tracked by https://github.com/johanvillalbab/agentsbar/issues/1823.

## Problem

On Claude Code 2.1.x, the `Claude Code-credentials` keychain item may contain only `mcpOAuth`. AgentsBar then fails to parse Claude OAuth credentials, treats the session as expired, and may periodically attempt delegated CLI refresh. That path can open the user's default browser from the background.
...
- [x] Release-built `AgentsBar.app` and packaged `AgentsBarCLI` isolated live proof
...
Fixes https://github.com/johanvillalbab/agentsbar/issues/1844. Primary OAuth storage discovery remains tracked by https://github.com/johanvillalbab/agentsbar/issues/1823.
