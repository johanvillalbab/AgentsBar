...
> **Unofficial integration:** T3 Chat does not publish an API for this data. AgentsBar reads a cookie-authenticated
...
AgentsBar imports your browser session cookie automatically. AgentsBar sends the cookie only to
`https://t3.chat`.
...
5. Paste the full `curl` command into the **T3 Chat cookie** field in AgentsBar settings.
...
```bash
# Show T3 Chat usage
agentsbar usage --provider t3chat

# Or use the alias
agentsbar usage --provider t3-chat
agentsbar usage --provider t3
```

T3 Chat provides no token-cost data. The `usage --format json` output contains usage and identity
data, while `agentsbar cost --provider t3chat` is unsupported.
...
| `T3 Chat session cookie is invalid or expired` | Stale or revoked session cookie | Sign out of T3 Chat, sign back in, then refresh AgentsBar or repaste the cookie |
...
| `Could not parse T3 Chat usage` | T3 Chat changed its tRPC response shape | Open a AgentsBar issue with a redacted response sample |
...
- `Sources/AgentsBarCore/Providers/T3Chat/T3ChatProviderDescriptor.swift` — provider metadata and fetch pipeline
- `Sources/AgentsBarCore/Providers/T3Chat/T3ChatUsageFetcher.swift` — tRPC request, cookie import, and cURL parsing
- `Sources/AgentsBarCore/Providers/T3Chat/T3ChatUsageSnapshot.swift` — response decoding and window mapping
- `Sources/AgentsBar/Providers/T3Chat/T3ChatProviderImplementation.swift` — settings pickers and bindings
- `Sources/AgentsBar/Providers/T3Chat/T3ChatSettingsStore.swift` — cookie source and header persistence
