...
- Newer cache shapes may omit `quotaUsage` but include `usage` counters. In that case AgentsBar derives
...
6. In AgentsBar: Providers → Windsurf → Cookie source → Manual → paste the JSON bundle.
...
- Parsed response fields used by AgentsBar:
...
- Sign in to [app.devin.ai](https://app.devin.ai) or [windsurf.com](https://windsurf.com) in Chrome, Edge, or another
  Chromium browser.
- Grant Full Disk Access to AgentsBar (System Settings → Privacy & Security → Full Disk Access).
...
## Key files
- `Sources/AgentsBarCore/Providers/Windsurf/WindsurfStatusProbe.swift` (local SQLite)
- `Sources/AgentsBarCore/Providers/Windsurf/WindsurfDevinSessionImporter.swift` (Chromium localStorage extraction)
- `Sources/AgentsBarCore/Providers/Windsurf/WindsurfWebFetcher.swift` (protobuf request + response parsing)
- `Sources/AgentsBarCore/Providers/Windsurf/WindsurfProviderDescriptor.swift` (fetch strategies)
- `Sources/AgentsBar/Providers/Windsurf/WindsurfProviderImplementation.swift` (settings UI)
- `Sources/AgentsBar/Providers/Windsurf/WindsurfSettingsStore.swift` (settings persistence)
