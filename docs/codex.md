...
- The menu and provider settings list every still-available expiry, while the optional credits setting controls
  nearing-expiry notifications. AgentsBar does not redeem or modify reset credits.
...
- Advanced users can add existing Codex homes to `~/.config/agentsbar/config.json` with
...
- AgentsBar reads identity from the configured home, exposes it in the Codex account switcher, and scopes
  remote Codex fetches with `CODEX_HOME`.
- Profile homes are not copied, reauthenticated, or removed by AgentsBar.
...
- Cached cookies: Keychain cache `com.steipete.agentsbar.cache` (account `cookie.codex`, source + timestamp).
...
- RPC reads are bounded: initialization has a longer startup budget, and normal requests have a shorter per-method
  timeout. On timeout, AgentsBar terminates the child `codex app-server` process so the stdout reader unwinds instead
...
- If macOS blocks or quarantines the `codex` executable, AgentsBar records the launch failure and skips background CLI
  launches for 30 minutes. Use a manual refresh after reinstalling or unblocking `codex` to retry immediately.
- AgentsBar also discovers the Codex CLI bundled with current ChatGPT and legacy Codex desktop apps, even when `codex`
...
- Manual/debug parser only; automatic background refresh and `AgentsBarCLI usage --source cli` do not launch bare Codex TUI.
...
- Cache:
  - Native + merged provider cache: `~/Library/Caches/AgentsBar/cost-usage/codex-v10.json`
...
## Key files
- Web: `Sources/AgentsBarCore/OpenAIWeb/*`
- CLI RPC + diagnostic PTY parser: `Sources/AgentsBarCore/UsageFetcher.swift`,
  `Sources/AgentsBarCore/Providers/Codex/CodexStatusProbe.swift`
