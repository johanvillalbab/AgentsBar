...
AgentsBar's Azure OpenAI provider validates that a configured deployment is reachable. It does not read Azure spend,
...
Settings -> Providers -> Azure OpenAI stores those values in the shared AgentsBar config. The same values can also be
...
```bash
printf '%s' "$AZURE_OPENAI_API_KEY" | agentsbar config set-api-key --provider azure-openai --stdin
```
...
AgentsBar sends a minimal chat-completions request to validate the deployment:
...
Set `AZURE_OPENAI_API_VERSION` to override the API version. When it is set to `v1`, AgentsBar uses Azure's
...
HTTPS. AgentsBar rejects explicit `http://` endpoints, user info, and encoded host-delimiter tricks before attaching the
`api-key` header.

Endpoint paths are preserved. AgentsBar avoids duplicating a trailing `/openai` for dated API versions or a trailing
...
```bash
agentsbar usage --provider azure-openai
agentsbar usage --provider azureopenai
agentsbar usage --provider aoai
```

## Key files

- `Sources/AgentsBarCore/Providers/AzureOpenAI/AzureOpenAIProviderDescriptor.swift`
- `Sources/AgentsBarCore/Providers/AzureOpenAI/AzureOpenAISettingsReader.swift`
- `Sources/AgentsBarCore/Providers/AzureOpenAI/AzureOpenAIUsageFetcher.swift`
- `Sources/AgentsBar/Providers/AzureOpenAI/AzureOpenAIProviderImplementation.swift`
- `Tests/AgentsBarTests/AzureOpenAIUsageFetcherTests.swift`
