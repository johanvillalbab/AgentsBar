...
1) **Coding Plan API token**
   - Set in Preferences → Providers → MiniMax (stored in `~/.config/agentsbar/config.json`), `MINIMAX_CODING_API_KEY`,
     or `MINIMAX_API_KEY`.
...
2) **Cached/imported browser session** (automatic web path)
   - Uses AgentsBar's standard cookie cache and browser import flow.
...
4) **Manual session cookie header** (optional web-path override)
   - Stored in `~/.config/agentsbar/config.json` via Preferences → Providers → MiniMax (Cookie source → Manual).
...
If the billing-history endpoint is unavailable but normal Coding Plan quota data is present, AgentsBar still shows the
quota card and omits the chart instead of treating the whole provider as failed.

## Key files
- `Sources/AgentsBarCore/Providers/MiniMax/MiniMaxUsageFetcher.swift`
- `Sources/AgentsBarCore/Providers/MiniMax/MiniMaxProviderDescriptor.swift`
- `Sources/AgentsBar/Providers/MiniMax/MiniMaxProviderImplementation.swift`
