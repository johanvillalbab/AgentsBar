...
1. Resolve a Factory API key from, in order:
   - `~/.config/agentsbar/config.json` `providers[].apiKey` for `factory` (also via Settings or
     `agentsbar config set-api-key --provider factory`)
   - `FACTORY_API_KEY`
...
1) **Cached cookie header** (Keychain cache `com.steipete.agentsbar.cache`, account `cookie.factory`).
2) **Stored session** (`~/Library/Application Support/AgentsBar/factory-session.json`).
...
## CLI
```bash
printf '%s' "$FACTORY_API_KEY" | agentsbar config set-api-key --provider factory --stdin
agentsbar usage --provider factory --source api
agentsbar usage --provider factory --source web
```
...
## Session storage
- File: `~/Library/Application Support/AgentsBar/factory-session.json`
...
## Troubleshooting
- Missing API key: set `FACTORY_API_KEY`, Settings → Droid → API key, or `agentsbar config set-api-key --provider factory`.
...
## Key files
- `Sources/AgentsBarCore/Providers/Factory/FactoryProviderDescriptor.swift`
- `Sources/AgentsBarCore/Providers/Factory/FactorySettingsReader.swift`
- `Sources/AgentsBarCore/Providers/Factory/FactoryStatusProbe.swift`
- `Sources/AgentsBarCore/Providers/Factory/FactoryLocalStorageImporter.swift`
