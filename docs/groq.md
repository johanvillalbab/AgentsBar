...
AgentsBar shows Groq organization usage and spend from the **console.groq.com** dashboard API, read through your
...
- Because the JWT cookie expires quickly and is refreshed by the SPA only while a console tab is open, AgentsBar
...
- Each activity row is per-model, per-day: `cost`, `n_context_tokens_total`, `n_non_cached_context_tokens_total`
  (cached = context − non-cached), `n_generated_tokens_total`, `num_requests`. AgentsBar aggregates these into daily
...
```bash
GROQ_SESSION_TOKEN=<stytch_session> agentsbar usage --provider groq --json
```

## Prometheus metrics (Enterprise, optional)

- Requires an Enterprise API key stored in `~/.config/agentsbar/config.json` (Settings → Providers → Groq) or `GROQ_API_KEY`.
...
- Console fetch: `Sources/AgentsBarCore/Providers/Groq/GroqConsoleFetcher.swift`
- Session import + JWT resolution: `Sources/AgentsBarCore/Providers/Groq/GroqConsoleSession.swift`
- Stytch refresh: `Sources/AgentsBarCore/Providers/Groq/GroqConsoleStytch.swift`
- Snapshot / cost-history projection: `Sources/AgentsBarCore/Providers/Groq/GroqConsoleUsageSnapshot.swift`
- Prometheus fallback: `Sources/AgentsBarCore/Providers/Groq/GroqUsageFetcher.swift`
- Provider wiring: `Sources/AgentsBarCore/Providers/Groq/GroqProviderDescriptor.swift`
