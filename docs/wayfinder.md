...
the cheap/local or the dearer/cloud tier serves it. AgentsBar polls the gateway's read-only
...
The integration is read-only. AgentsBar never sends prompts through the gateway, never calls
...
in to: enable the provider in AgentsBar Settings → Providers → Wayfinder. The default gateway
...
- Routing split over the last 30 days: requests per configured route (up to 5, by request
  ...
  asserting which route is "local," so AgentsBar never guesses.
...
If the gateway is not running, AgentsBar reports that it could not be reached and suggests
