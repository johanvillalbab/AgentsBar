...
Related: [#1756](https://github.com/johanvillalbab/agentsbar/issues/1756),
[#1268](https://github.com/johanvillalbab/agentsbar/issues/1268), and the bounded Claude sign-in repair in
[#1811](https://github.com/johanvillalbab/agentsbar/pull/1811).
...
This solves the durable OAuth refresh problem without making AgentsBar a second credential vault. It also avoids a
...
AgentsBar has three account concepts today:

- The ambient Claude OAuth credential is routed from AgentsBar's cache, Claude Code's credentials file, or Claude
  Code's Keychain item. It represents one active credential. Claude Code-owned expired credentials delegate refresh
  back to the CLI; AgentsBar-owned cached credentials can refresh directly.
...
The recently merged [#1800](https://github.com/johanvillalbab/agentsbar/pull/1800) scopes Claude OAuth history to the routed
Keychain identity. [#1776](https://github.com/johanvillalbab/agentsbar/pull/1776) prevents CLI-runtime usage refreshes from
...
The closed [#1707](https://github.com/johanvillalbab/agentsbar/pull/1707) should not be revived. It coupled account discovery,
...
| --- | --- | --- | --- | --- |
| First-party OAuth account vault | AgentsBar | High | New login, refresh, storage, revocation, migration, and security surface | Defer |
...
| Existing token accounts | AgentsBar config | Low for OAuth | Access token expires without refresh metadata | Keep for current cookie/API-key uses |
...
envelope. AgentsBar does not need
...
- AgentsBar never reads `claude-swap` storage, Claude Code storage, environment credentials, or Keychain entries. The
...
last refresh, adapter errors, and a link to the upstream project; AgentsBar should not install or update it.
...
the account action is now named “Sign in with Claude Code…” and no longer claims it will add a durable AgentsBar account.
