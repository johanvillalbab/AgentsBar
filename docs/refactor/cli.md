...
read_when:
  - "Refactoring AgentsBar CLI error handling or config parsing."
...
- (Optional) `agentsbar config dump`
  - Prints normalized config JSON.

## Step-by-step implementation guide
1. **Add validation types**
   - `AgentsBarConfigIssue` + `AgentsBarConfigValidator` in `AgentsBarCore/Config`.
   - Keep file <500 LOC.
...
8. **Verification**
   ...
   - CLI e2e: `agentsbar --json-only ...`, `agentsbar config validate`.
