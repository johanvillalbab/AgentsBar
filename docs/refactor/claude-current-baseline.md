...
This document is the current-state parity reference for Claude behavior in AgentsBar.
...
This behavior is owned by `Sources/AgentsBarCore/Providers/Claude/ClaudeProviderDescriptor.swift`
...
- CLI-side Claude token-account behavior:
  - OAuth token account changes the effective source mode from `auto` to `oauth`, disables cookie mode, omits a
    manual cookie header, and injects `AGENTSBAR_CLAUDE_OAUTH_TOKEN`.
...
This behavior is implemented in `Sources/AgentsBarCore/Providers/Claude/ClaudeUsageFetcher.swift`
...
- `Tests/AgentsBarTests/ClaudeBaselineCharacterizationTests.swift`
- `Tests/AgentsBarTests/ClaudeOAuthFetchStrategyAvailabilityTests.swift`
- `Tests/AgentsBarTests/ClaudeUsageTests.swift`
- `Tests/AgentsBarTests/TokenAccountEnvironmentPrecedenceTests.swift`
- `Tests/AgentsBarTests/SettingsStoreCoverageTests.swift`
