...
1. **Authentication** — Three methods (in priority order):
   - **Auto mode**: Username + password entered in Settings → Providers → StepFun.
     AgentsBar performs a 3-step login flow to obtain an Oasis-Token:
...
- `Sources/AgentsBarCore/Providers/StepFun/StepFunProviderDescriptor.swift` (descriptor + web fetch strategy)
- `Sources/AgentsBarCore/Providers/StepFun/StepFunUsageFetcher.swift` (login flow + HTTP client + JSON parser)
- `Sources/AgentsBarCore/Providers/StepFun/StepFunSettingsReader.swift` (env var resolution)
- `Sources/AgentsBar/Providers/StepFun/StepFunProviderImplementation.swift` (settings fields + activation logic)
- `Sources/AgentsBar/Providers/StepFun/StepFunSettingsStore.swift` (SettingsStore extension)
- `Tests/AgentsBarTests/StepFunUsageFetcherTests.swift` (26 test cases)
