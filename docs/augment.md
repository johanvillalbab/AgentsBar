...
Cached cookies:
- Keychain cache `com.steipete.agentsbar.cache` (account `cookie.augment`, source + timestamp). Reused before re-importing
  from browsers.

### Auggie CLI Integration

If the `auggie` CLI is installed, AgentsBar will prefer `auggie account status` for usage data and avoid browser prompts.
When the CLI is unavailable or not authenticated, AgentsBar falls back to browser cookies.
...
3. Return to AgentsBar and click "Refresh" in the menu
...
3. Return to AgentsBar - it will automatically import fresh cookies
...
5. In AgentsBar: **Settings → Providers → Augment → Cookie Source → Manual**
