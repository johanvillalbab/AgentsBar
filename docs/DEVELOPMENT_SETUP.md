...
When developing AgentsBar, you may see frequent keychain permission prompts like:

> **AgentsBar wants to access key "Claude Code-credentials" in your keychain.**

This happens because each rebuild creates a new code signature, and macOS treats it as a "different" app.
That can affect both AgentsBar-owned entries (`com.steipete.AgentsBar`, `com.steipete.agentsbar.cache`) and
...
This creates a self-signed certificate named "AgentsBar Development".

#### 2. Trust the Certificate

1. Open **Keychain Access.app**
2. Find **"AgentsBar Development"** in the **login** keychain
3. Double-click it
...
```bash
export APP_IDENTITY='AgentsBar Development'
```
...
> Note: `compile_and_run.sh` now auto-detects a valid signing identity (Developer ID or AgentsBar Development).
...
If you see multiple `AgentsBar *.app` bundles in your project directory, you can clean them up:
...
The build script creates `AgentsBar.app` in the project root. Old numbered builds (like `AgentsBar 2.app`) are created when Finder can't overwrite the running app.
...
This script:
1. Kills existing AgentsBar instances
...
5. Launches `AgentsBar.app`
6. Verifies it stays running

Launching an unbundled `AgentsBar` executable, including SwiftPM builds using `.build` or a custom scratch path, disables
Keychain access for that process to avoid repeated password prompts. Use the packaged `AgentsBar.app` when local
...
When the script falls back to ad-hoc signing, it preserves AgentsBar-owned keychain state by default.
That means you may still see keychain prompts for existing AgentsBar cache entries, but allowing those prompts keeps the
cached browser/OAuth state available across normal rebuilds.
If you want a clean reset of AgentsBar-owned keychain state for an ad-hoc build, run
