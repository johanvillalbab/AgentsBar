...
AgentsBar can list live Codex and Claude Code sessions on this Mac and other Macs or Linux hosts reachable over SSH.
...
Thread titles can contain sensitive text. **Project** remains the default; choose a descriptive mode only if you are comfortable showing those titles in the menu. AgentsBar reads title metadata without modifying Codex state and does not persist it to disk.
...
The menu groups local sessions first, followed by each remote host. A filled dot is active; an empty dot is idle. Select a local row to activate its terminal, editor, or desktop app. The first focus attempt can request macOS Accessibility permission so AgentsBar can raise the matching window. Remote rows run the same focus command over SSH.
...
agentsbar sessions focus <session-id>
```

Remote hosts need key-based, non-interactive SSH and either `agentsbar` on `PATH` or AgentsBar installed in `/Applications`.
