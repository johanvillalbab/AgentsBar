...
Issue: [#1735](https://github.com/johanvillalbab/agentsbar/issues/1735)
...
1. a config file chooses where AgentsBar sends a secret;
...
- `authentication`: `none`, `bearer`, or `x-api-key`; the secret is never inline. Authenticated instances read only their
  derived variable `AGENTSBAR_CUSTOM_<INSTANCE_ID>_API_KEY`, with the ID uppercased and hyphens replaced by underscores.
...
confirmation must state this clearly. AgentsBar still must contain the service response and must never disclose unrelated
credentials.
...
4. **Config and CLI integration:** version-2 migration, `agentsbar config validate`, local approval records and interactive
