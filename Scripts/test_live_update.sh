...
ZIP_URL="https://github.com/johanvillalbab/agentsbar/releases/download/${PREV_TAG}/${APP_NAME}-macos-universal-${PREV_VER}.zip"
TMP_DIR=$(mktemp -d /tmp/agentsbar-live.XXXX)
...
osascript -e 'tell application "AgentsBar" to quit' >/dev/null 2>&1 || true
for _ in {1..20}; do
