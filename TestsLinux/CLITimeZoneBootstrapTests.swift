...
struct CLITimeZoneBootstrapTests {
    ...
    func `derives IANA identifier from resolved zoneinfo path`() {
        #expect(AgentsBarCLI.linuxTimeZoneBootstrapIdentifier(
            currentValue: nil,
    ...
    func `normalizes conventional zoneinfo paths`(resolvedPath: String, expectedIdentifier: String) {
        #expect(AgentsBarCLI.linuxTimeZoneIdentifier(from: resolvedPath) == expectedIdentifier)
    }
    ...
    func `rejects invalid or unrelated resolved paths`(resolvedPath: String) {
        #expect(AgentsBarCLI.linuxTimeZoneIdentifier(from: resolvedPath) == nil)
    }

    @Test
    func `rejects invalid CoreFoundation timezone data`() {
        #expect(!AgentsBarCLI.primeCoreFoundationTimeZone(
            identifier: "Etc/AgentsBarInvalid",
            filePath: "/dev/null"))
    }
    ...
    func `primes the legacy formatter bridge with system timezone data`() throws {
        ...
        guard let identifier = AgentsBarCLI.linuxTimeZoneIdentifier(from: resolvedPath) else { return }
