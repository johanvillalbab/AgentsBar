...
struct HooksTests {
    ...
    func `environment variables include set fields and omit nil`() {
        ...
        #expect(env["AGENTSBAR_EVENT"] == "quota_low")
        #expect(env["AGENTSBAR_PROVIDER"] == "codex")
        #expect(env["AGENTSBAR_WINDOW"] == "weekly")
        #expect(env["AGENTSBAR_USAGE_PERCENT"] == "0.5")
        #expect(env["AGENTSBAR_RESET_AT"] == "2023-11-14T22:13:20Z")
        #expect(env["AGENTSBAR_TIMESTAMP"] != nil)
        #expect(env["AGENTSBAR_ACCOUNT"] == nil)
        #expect(env["AGENTSBAR_STATUS"] == nil)
    }
    ...
    func `runner executes command and passes event environment`() async throws {
        ...
        #expect(result.stdout.contains("AGENTSBAR_EVENT=quota_reached"))
        #expect(result.stdout.contains("AGENTSBAR_PROVIDER=codex"))
    }
    ...
    func `runner throws on missing executable`() async {
        let rule = HookRule(event: .quotaReached, executable: "/nonexistent/agentsbar-hook")
