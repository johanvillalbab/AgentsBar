...
- CLI: add quota-aware agentsbar guard automation gates with stable exit codes, explicit windows, JSON output, and bounded fetches (#2237). Thanks @OfficialAbhinavSingh!
...
- CLI: add token-gated dashboard snapshots to agentsbar serve, keep sensitive responses uncached, and require explicit plain-HTTP opt-in for LAN binds (#2227). Thanks @jethac!
...
- Community integrations: list agentsbar-plasmoid, a KDE Plasma 6 usage widget. Thanks @psimaker!
...
- Claude OAuth: remember an acknowledged AgentsBar Keychain explanation for six hours without suppressing macOS authorization or either Keychain opt-out (#1990). Thanks @harjothkhara!
...
- Claude: prevent AgentsBar's passive CLI probes from starting background Claude Code updates, avoiding repeated partial downloads when a probe exits before an update completes. Thanks @PG2047!
...
### Added
- CLI: add responsive `agentsbar cards` and compact `--brief` terminal usage views. Thanks @DonnieFi!
...
- Debug builds: add a compact `D` beside menu-bar icons and identify them as AgentsBar Debug in tooltips and accessibility.
...
### Changed
- Branding: replace the app and website icon with a usage-meter prompt mark that matches AgentsBar's core UI.
- Website: redesign agentsbar.app around faster download, provider discovery, feature, CLI, and widget paths with responsive dark/light and localized layouts. Thanks @vyctorbrzezowski!
...
- Codex agents: add a read-only `agentsbar` skill for bounded, redacted provider usage JSON. Thanks @coygeek!
...
- CLI server: reload provider config for every usage and cost request, invalidate config-dependent cache entries, and prune expired config variants without restarting `agentsbar serve`. Thanks @enieuwy!
...
- Claude: remove transient ClaudeProbe session artifacts after CLI usage polls so background refreshes no longer fill Claude Code project history with AgentsBar `/usage` sessions (#1301). Thanks @LPFchan and @matthewod11-stack!
...
### Fixed
- Menu bar: stop forcing a private preferred-position value for fresh status items; suspicious stored positions are now cleared so AppKit can place AgentsBar normally on macOS 26 / 5K displays (#1267). Thanks @AdrianSimionov, @kirocop, and @Yuxin-Qiao!
...
### Added
- QA: document the live AgentsBar e2e flow and add a redacted provider-matrix helper for packaged CLI smoke tests.
...
### Fixed
- Claude: keep Claude CLI-owned OAuth refresh tokens delegated to Claude Code when CLI storage is present, preventing AgentsBar from consuming rotating refresh tokens and forcing re-login (#1161, #1239). Thanks @RajvardhanPatil07!
...
- Menu bar: give AgentsBar status items stable placement identities while preserving existing upgrade placement state (#1216). Thanks @pdurlej!
...
### Changed
- Docs: update the Homebrew install command to use the official `agentsbar` cask now that it supports Intel Macs (#1189). Thanks @SSakutaro!
...
### Fixed
- CLI: bound `agentsbar serve` requests with a configurable timeout and coalesce concurrent cache misses so hung `/usage` callers no longer stampede provider refreshes (#1208). Thanks @enieuwy!
...
### Changed
- CLI: make `agentsbar diagnose` use a generic safe provider diagnostic export for all providers, with MiniMax details attached only as provider-specific metadata.
...
- Menu bar: clear stale hidden macOS status-item visibility defaults once before creating AgentsBar items (#1169).
...
- Menu bar: preserve status item identity during display-change recovery so menu bar managers do not treat AgentsBar as a new hidden item (#1122, fixes #1109). Thanks @lederniermagicien!
...
- Release: update Homebrew automation so AgentsBar releases publish both the CLI formula and app cask from the same workflow.
...
- CLI: add `agentsbar config set-api-key` for safely storing provider API keys from stdin.
- CLI: add `agentsbar config providers`, `enable`, and `disable` for scripting the same provider toggles used by Settings.
- CLI: let `--all-accounts` and `agentsbar serve` export every visible Codex account instead of only the selected account (#1019).
...
- CLI: reject non-loopback `Host` headers in `agentsbar serve` before serving local usage and cost metadata (#995). Thanks @rohitjavvadi!
...
- CLI: add `agentsbar serve` for localhost JSON access to usage and cost endpoints (#957). Thanks @ThiagoCAltoe!
...
- Menu bar: detect when macOS Tahoe hides AgentsBar behind the new Allow in Menu Bar setting and show recovery guidance (#945, fixes #890). Thanks @pdurlej!
...
- Settings: let stale managed Codex account records be removed even when their stored home path is outside AgentsBar's managed-home directory, and keep CLI known-owner tests from writing fixtures into the live app store.
...
- Keychain cache: reduce repeated development prompt churn by trusting the bundled helper when writing AgentsBar-owned cache items (#888).
...
- Keychain cache: trust the bundled AgentsBarCLI helper when writing AgentsBar-owned cache items, reducing repeated "AgentsBar Cache" prompts from CLI usage (#679). Thanks @QuarkAssistant!
...
- Build script: make AgentsBar-owned ad-hoc keychain cleanup opt-in with `--clear-adhoc-keychain`, and extend the explicit reset path to clear both `com.steipete.AgentsBar` and `com.steipete.agentsbar.cache`. Thanks @magnaprog!
...
### Claude OAuth & Keychain
- Claude OAuth creds are cached in AgentsBar Keychain to reduce repeated prompts.
...
- CLI/App: move provider secrets + ordering to `~/.config/agentsbar/config.json` (no Keychain persistence).
...
- CLI: add `agentsbar cost` to print local cost usage (text/JSON) for Codex + Claude.
...
- macOS: AgentsBar now supports Intel Macs (x86_64 builds + Sonoma fallbacks). Thanks @epoyraz!
...
- Homebrew: cask now installs the `agentsbar` CLI symlink. Thanks @dalisoft!
...
- Widgets: add a new “AgentsBar Switcher” widget that lets you switch providers and remember the selection.
...
- CI: build/test Linux `AgentsBarCLI` (x86_64 + aarch64) and publish release assets as `AgentsBarCLI-<tag>-linux-<arch>.tar.gz` (+ `.sha256`).
...
## 0.8.0 — 2025-12-17
- AgentsBar is now available via Homebrew: `brew install --cask steipete/tap/agentsbar` (updates via `brew upgrade --cask steipete/tap/agentsbar`).
...
## 0.6.0 — 2025-12-04
- New bundled CLI (`agentsbar`) with single `usage` command, `--format text|json`, `--status`, and fast `-h/-V`.
- CLI output now shows consistent headers (`Codex 0.x.y (codex-cli)`, `Claude Code <ver> (claude)`) and JSON includes `source` + `status`.
- Advanced prefs install button symlinks `agentsbar` into /usr/local/bin and /opt/homebrew/bin; docs refreshed.
...
- Debug replay toggle (`defaults write com.steipete.agentsbar debugMenuEnabled -bool YES`) to view every pattern.
...
- Notarized release build "AgentsBar-0.1.0.zip" remains current artifact; app version 0.1.1.

## 0.1.0 — 2025-11-16
- Initial AgentsBar release: macOS 15+ menu bar app, no Dock icon.
