...
struct ServeOptions: CommanderParsable {
    ...
    @Option(
        ...
        help: "Bearer token for /dashboard/v1/snapshot (prefer AGENTSBAR_DASHBOARD_TOKEN)")
...
struct ServeRuntime {
    let configStore: AgentsBarConfigStore
    ...
    init(
        configStore: AgentsBarConfigStore,
...
extension AgentsBarCLI {
    ...
    static func runServe(_ values: ParsedValues) async {
        ...
        do {
            try await server.run {
                Self.writeStderr("AgentsBar server listening on http://\(bindHost):\(port)\n")
    ...
    static let dashboardTokenEnvironmentVariable = "AGENTSBAR_DASHBOARD_TOKEN"
                    ...
                    await Self.serveDashboardSnapshot(
                        context: ServeDashboardContext(
                            ...
                            costCollection: ServeCostCollectionContext(
                                ...
                                now: { ContinuousClock().now },
                                providerOperations: runtime.costOperations),
                            agentsBarVersion: runtime.healthVersion))
                }))
        }
    }

    static func loadServeConfigSnapshot(
        configStore: AgentsBarConfigStore = AgentsBarConfigStore()) throws -> CLIServeConfigSnapshot
    {
        let config = try configStore.load() ?? AgentsBarConfig.makeDefault()
    ...
    static func serveConfigCacheToken(for config: AgentsBarConfig) throws -> String {
    ...
    static func cachedServeResponse(
        ...
        cache: CLIServeResponseCache,
        refreshInterval: TimeInterval,
        requestTimeout: TimeInterval = AgentsBarCLI.defaultServeRequestTimeout,
    ...
    private static func serveDashboardSnapshot(context: ServeDashboardContext) async -> CLILocalHTTPResponse {
        ...
        let snapshot = DashboardSnapshotBuilder.makeSnapshot(
            ...
            agentsBarVersion: context.agentsBarVersion)
    ...
    private static func serveProviderSelection(
        rawProvider: String?,
        config: AgentsBarConfig) throws -> ProviderSelection
    {
