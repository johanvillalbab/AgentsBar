...
extension AgentsBarCLI {
    static func decodeProvider(from values: ParsedValues, config: AgentsBarConfig) -> ProviderSelection {
    ...
    static func resetTimeDisplayStyleFromDefaults() -> ResetTimeDisplayStyle {
        let domains = [
            "com.steipete.agentsbar",
            "com.steipete.agentsbar.debug",
        ]
    ...
    static func loadConfig(output: CLIOutputPreferences) -> AgentsBarConfig {
        let store = AgentsBarConfigStore()
        do {
            if let existing = try store.load() {
                return existing
            }
            return AgentsBarConfig.makeDefault()
        } catch {
