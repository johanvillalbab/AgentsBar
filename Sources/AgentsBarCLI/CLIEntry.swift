...
enum AgentsBarCLI {
    ...
    private static func bootstrapLogging(path: [String], values: ParsedValues) {
        AgentsBarLog.bootstrapIfNeeded(self.loggingConfiguration(path: path, values: values))
    }

    static func loggingConfiguration(path: [String], values: ParsedValues) -> AgentsBarLog.Configuration {
        ...
        let destination: AgentsBarLog.Destination = path == ["diagnose"] ? .discard : .stderr
        return .init(destination: destination, level: level, json: isJSON)
    }

    static func resolvedLogLevel(verbose: Bool, rawLevel: String?) -> AgentsBarLog.Level {
        AgentsBarLog.parseLevel(rawLevel) ?? (verbose ? .debug : .error)
    }
