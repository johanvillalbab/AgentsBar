...
public enum KeychainCacheStore {
    ...
    private static let log = AgentsBarLog.logger(LogCategories.keychainCache)
    private static let cacheService = "com.steipete.agentsbar.cache"
    private static let legacyCacheService = "com.steipete.codexbar.cache"
    private static let cacheLabel = "AgentsBar Cache"
        ...
        as type: Entry.Type = Entry.self) -> LoadResult<Entry>
    {
        ...
        switch status {
        case errSecSuccess:
            return self.decodeLoadedEntry(result: result, key: key)
        case errSecItemNotFound:
            query[kSecAttrService as String] = self.legacyCacheService
            var legacyResult: AnyObject?
            let legacyStatus = KeychainSecurity.copyMatching(query as CFDictionary, &legacyResult)
            switch legacyStatus {
            case errSecSuccess:
                return self.decodeLoadedEntry(result: legacyResult, key: key)
            default:
                return self.loadResultForKeychainReadFailure(status: legacyStatus, key: key)
            }
        default:
            return self.loadResultForKeychainReadFailure(status: status, key: key)
        }
    ...
    #if os(macOS)
    private static func decodeLoadedEntry<Entry: Codable>(
        result: AnyObject?,
        ...
        fileExists: (String) -> Bool = { FileManager.default.fileExists(atPath: $0) }) -> [String]
    {
        ...
        if let appBundle {
            ...
            append(appBundle.appendingPathComponent("Contents/Helpers/AgentsBarCLI").path)
        }
