...
final class OpenAIDashboardWebViewCache {
    ...
    fileprivate static let log = AgentsBarLog.logger(LogCategories.openAIWebview)
    ...
    (() => {
      try {
        delete window.__agentsbarDidScrollToCredits;
        delete window.__agentsbarUsageBreakdownJSON;
        delete window.__agentsbarUsageBreakdownDebug;
        return true;
      } catch {
        return false;
      }
