...
public struct RemoteSessionFetcher: Sendable {
    public static let bundledCLIFallback = "/Applications/AgentsBar.app/Contents/Helpers/AgentsBarCLI"
        ...
        environment: [String: String] = ProcessInfo.processInfo.environment) async
    {
        ...
        let command = "agentsbar sessions focus \(Self.shellQuote(sessionID)) || " +
    ...
    private func fetch(host: String, environment: [String: String]) async -> RemoteSessionHostResult {
        ...
        let command = "agentsbar sessions --json || " +
    ...
    /// wrapper, but a GUI-launched AgentsBar inherits a minimal `PATH` (`/usr/bin:/bin`)
