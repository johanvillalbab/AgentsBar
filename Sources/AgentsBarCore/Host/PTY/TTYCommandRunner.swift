...
public struct TTYCommandRunner {
    private static let log = AgentsBarLog.logger(LogCategories.ttyRunner)
    ...
    public enum Error: Swift.Error, LocalizedError, Sendable {
        ...
        public var errorDescription: String? {
            ...
            case .outputTooLarge: "PTY command produced more output than AgentsBar can safely process."
            }
        }
    }
    ...
    static func locateBundledHelper(_ name: String) -> String? {
        ...
        if let override = ProcessInfo.processInfo.environment["AGENTSBAR_HELPER_\(name.uppercased())"],
        ...
        onURLDetected: (@Sendable () -> Void)? = nil) throws -> Result
    {
        ...
        if isClaudeCLI,
           let watchdog = Self.locateBundledHelper("AgentsBarClaudeWatchdog")
        {
            executable = watchdog
