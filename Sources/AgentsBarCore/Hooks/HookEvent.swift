...
/// The quota/provider state changes AgentsBar can run external commands for.
///
...
public struct HookEvent: Codable, Sendable, Equatable {
    ...
    public func environmentVariables() -> [String: String] {
        var env: [String: String] = [
            "AGENTSBAR_EVENT": self.event.rawValue,
            "AGENTSBAR_PROVIDER": self.provider,
            "AGENTSBAR_TIMESTAMP": Self.iso8601String(self.timestamp),
        ]
        if let account { env["AGENTSBAR_ACCOUNT"] = account }
        if let window { env["AGENTSBAR_WINDOW"] = window }
        if let usagePercent { env["AGENTSBAR_USAGE_PERCENT"] = Self.number(usagePercent) }
        if let used { env["AGENTSBAR_USED"] = Self.number(used) }
        if let limit { env["AGENTSBAR_LIMIT"] = Self.number(limit) }
        if let resetAt { env["AGENTSBAR_RESET_AT"] = Self.iso8601String(resetAt) }
        if let status { env["AGENTSBAR_STATUS"] = status }
        return env
    }
