...
public enum CursorCookieImporter {
    ...
    /// profile that AgentsBar cannot read. Ordinary background imports remain stricter.
        ...
        logger: ((String) -> Void)?) -> [SessionInfo]
    {
        ...
        do {
            ...
            let sources = try Self.cookieClient.agentsBarRecords(
                matching: query,
                in: browser,
                logger: log)
...
struct CursorAppAuthStore: CursorAppAuthSessionProviding {
        ...
        fileManager: FileManager = .default) -> String
    {
        ...
        #elseif os(Linux)
        let configHome = environment[AgentsBarConfigStore.xdgConfigHomeEnvironmentKey]?
...
public enum CursorStatusProbeError: LocalizedError, Sendable {
    ...
    static let safariFullDiskAccessHint =
        "If you use Safari, grant AgentsBar Full Disk Access in System Settings ▸ Privacy & Security."

    public var errorDescription: String? {
        switch self {
        case .notLoggedIn:
            #if os(macOS)
            "Not logged in to Cursor. Please log in via the AgentsBar menu."
            #else
            "Not logged in to Cursor. Sign in to the Cursor app on this machine or paste a Cookie header copied "
                + "from cursor.com into ~/.config/agentsbar/config.json (legacy: ~/.config/agentsbar/config.json)."
            #endif
        ...
        case .noSessionCookie:
            ...
            "No Cursor session found. \(Self.safariFullDiskAccessHint) "
                ...
                + "You can also sign in to Cursor from the AgentsBar menu (Add / switch account)."
            #else
...
public actor CursorSessionStore {
    ...
    private init() {
        ...
        let dir = appSupport.appendingPathComponent("AgentsBar", isDirectory: true)
