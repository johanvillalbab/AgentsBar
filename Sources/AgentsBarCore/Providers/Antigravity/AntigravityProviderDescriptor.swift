...
/// process. ``agy`` is an interactive REPL, not a query command, so AgentsBar
...
struct AntigravityCLIHTTPSFetchStrategy: ProviderFetchStrategy {
    ...
    private static let log = AgentsBarLog.logger(LogCategories.antigravity)
    ...
    struct WarmAgyDependencies {
        ...
        /// The pid of an ``agy`` that AgentsBar itself spawned and manages through
        ...
        /// long-lived `agy`, or another AgentsBar host) has no such accounting.
        ...
        dependencies: WarmAgyDependencies) async throws -> AntigravityStatusSnapshot?
    {
        ...
        // Also exclude any `agy` AgentsBar itself spawned and manages: reusing it
    ...
    static func liveWarmAgyDependencies() -> WarmAgyDependencies {
        WarmAgyDependencies(
            ...
            ownedPID: {
                // The pid of the `agy` AgentsBar manages through the shared
        ...
        async throws -> ProviderFetchResult
    {
        ...
        // process (an IDE, a long-lived `agy`, or another AgentsBar host), so
        // AgentsBar must not manage its lifecycle, idle timeout, or
        // `resetAfterFetch` teardown. Those apply only to processes AgentsBar
    ...
    /// Spawn (or reuse AgentsBar's own warm) `agy` session and wait for the CLI
    ...
    static func shouldResetSessionAfterFetch(_ context: ProviderFetchContext) -> Bool {
        // Long-lived hosts (the app, `agentsbar serve`) keep the warm `agy`
