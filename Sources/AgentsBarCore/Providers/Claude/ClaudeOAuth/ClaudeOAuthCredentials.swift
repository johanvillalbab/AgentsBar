...
public enum ClaudeOAuthCredentialsStore {
    ...
    public static let environmentTokenKey = "AGENTSBAR_CLAUDE_OAUTH_TOKEN"
    public static let environmentScopesKey = "AGENTSBAR_CLAUDE_OAUTH_SCOPES"
    ...
    public static let environmentClientIDKey = "AGENTSBAR_CLAUDE_OAUTH_CLIENT_ID"
    ...
    static let log = AgentsBarLog.logger(LogCategories.claudeUsage)
    ...
    private static let pendingAgentsBarOAuthKeychainCacheClearKey =
        "ClaudeOAuthPendingAgentsBarOAuthKeychainCacheClearV1"
    private static let pendingAgentsBarOAuthKeychainCacheClearStore: ClaudeOAuthPendingCacheClearStore =
        ClaudeOAuthPendingCacheClearUserDefaultsStore(
            ...
            domain: "com.steipete.agentsbar",
            key: ClaudeOAuthCredentialsStore.pendingAgentsBarOAuthKeychainCacheClearKey)
    ...
    private struct Repository {
            ...
            allowClaudeKeychainRepairWithoutPrompt: Bool) throws -> ClaudeOAuthCredentialRecord
        {
            try self.context.run {
                ...
                if ClaudeOAuthCredentialsStore.shouldUseAgentsBarOAuthKeychainCache,
                   !ClaudeOAuthCredentialsStore.hasPendingAgentsBarOAuthKeychainCacheClear,
                ...
                switch ClaudeOAuthCredentialsStore.loadAgentsBarOAuthKeychainCache() {
        ...
        private func validCachedCredentialAfterWaitingForPromptLock() -> ClaudeOAuthCredentialRecord? {
            ...
            guard case let .found(entry) = ClaudeOAuthCredentialsStore.loadAgentsBarOAuthKeychainCache(),
        ...
        private func resolvedCacheOwner(_ owner: ClaudeOAuthCredentialOwner) -> ClaudeOAuthCredentialOwner {
            guard owner == .agentsbar else { return owner }
        ...
        func invalidateCacheIfCredentialsFileChanged() -> Bool {
            self.context.run {
                ...
                if ClaudeOAuthCredentialsStore.shouldUseAgentsBarOAuthKeychainCache {
                    if ClaudeOAuthCredentialsStore.hasPendingAgentsBarOAuthKeychainCacheClear {
                    ...
                    ClaudeOAuthCredentialsStore.markPendingAgentsBarOAuthKeychainCacheClear()
                }
        ...
        func shouldAttemptFreshnessSyncFromClaudeKeychain(cached: ClaudeOAuthCredentialRecord) -> Bool {
            ...
            case .onlyOnUserAction:
                if ProviderInteractionContext.current != .userInitiated {
                    if ProcessInfo.processInfo.environment["AGENTSBAR_DEBUG_CLAUDE_OAUTH_FLOW"] == "1" {
                        ClaudeOAuthCredentialsStore.log.debug(
    ...
    /// - AgentsBar-owned credentials refresh directly via token endpoint.
    ...
    /// Save refreshed credentials to AgentsBar's keychain cache
        ...
        historyOwnerIdentifier: String?)
    {
        ...
        self.saveToCacheKeychain(
            ...
            owner: .agentsbar,
            historyOwnerIdentifier: historyOwnerIdentifier)
        self.log.debug("Saved refreshed credentials to AgentsBar keychain cache")
    }
    ...
    /// Check if AgentsBar has cached credentials (in memory or keychain cache)
    ...
    private static func saveToCacheKeychain(
        ...
        historyOwnerIdentifier: String? = nil)
    {
        guard self.shouldUseAgentsBarOAuthKeychainCache else {
            self.markPendingAgentsBarOAuthKeychainCacheClear()
            return
        }
        let entry = CacheEntry(
            ...
            owner: owner,
            historyOwnerIdentifier: historyOwnerIdentifier)
        self.currentPendingAgentsBarOAuthKeychainCacheClearStore.withCacheTransaction { pending in
            if pending {
    ...
    private static func clearCacheKeychain() {
        if self.shouldUseAgentsBarOAuthKeychainCache {
            self.currentPendingAgentsBarOAuthKeychainCacheClearStore.withCacheTransaction { pending in
                switch KeychainCacheStore.clearResult(key: self.cacheKey) {
                case .removed, .missing:
                    pending = false
    ...
    private static func loadAgentsBarOAuthKeychainCache() -> KeychainCacheStore.LoadResult<CacheEntry> {
        guard self.shouldUseAgentsBarOAuthKeychainCache else { return .missing }
    ...
    private static var shouldUseAgentsBarOAuthKeychainCache: Bool {
        ClaudeOAuthKeychainPromptPreference.storedMode() != .never
    }

    private static func markPendingAgentsBarOAuthKeychainCacheClear() {
        self.currentPendingAgentsBarOAuthKeychainCacheClearStore.markPending()
    }

    private static var hasPendingAgentsBarOAuthKeychainCacheClear: Bool {
        self.currentPendingAgentsBarOAuthKeychainCacheClearStore.isPending
    }

    private static var currentPendingAgentsBarOAuthKeychainCacheClearStore: ClaudeOAuthPendingCacheClearStore {
        #if DEBUG
        if let store = self.taskPendingCacheClearStoreOverride {
            return store
        }
        #endif
        return self.pendingAgentsBarOAuthKeychainCacheClearStore
    }
...
extension ClaudeOAuthCredentialsStore {
    ...
    /// AgentsBar's caches. This is used to avoid triggering a second OS keychain dialog during the OAuth retry.
    @discardableResult
    ...
    /// Updates AgentsBar's keychain cache with the new credentials.
