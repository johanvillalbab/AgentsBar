...
actor ClaudeCLISession {
    ...
    private static let log = AgentsBarLog.logger(LogCategories.claudeCLI)
    private static let probeSessionIDFilename = ".agentsbar-session-id"
    ...
    enum SessionError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .outputTooLarge: "Claude CLI session produced more output than AgentsBar can safely process."
            }
        }
    }
    ...
    private func ensureStarted(binary: String) throws {
        ...
        let disableWatchdog = ProcessInfo.processInfo.environment["AGENTSBAR_DISABLE_CLAUDE_WATCHDOG"] == "1"
        if !disableWatchdog,
           resolvedURL.lastPathComponent == "claude",
           let watchdog = TTYCommandRunner.locateBundledHelper("AgentsBarClaudeWatchdog")
        {
