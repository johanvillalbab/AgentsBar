...
struct AgentsBarWidgetEntry: TimelineEntry {
    let date: Date
    let provider: UsageProvider
    let snapshot: WidgetSnapshot
}

struct AgentsBarCompactEntry: TimelineEntry {
    let date: Date
    let provider: UsageProvider
...
struct AgentsBarSwitcherEntry: TimelineEntry {
    let date: Date
    let provider: UsageProvider
...
struct AgentsBarTimelineProvider: AppIntentTimelineProvider {
    func placeholder(in context: Context) -> AgentsBarWidgetEntry {
        AgentsBarWidgetEntry(
            date: Date(),
            provider: .codex,
            snapshot: WidgetPreviewData.snapshot())
    }

    func snapshot(for configuration: ProviderSelectionIntent, in context: Context) async -> AgentsBarWidgetEntry {
        let provider = configuration.provider.provider
        return AgentsBarWidgetEntry(
            date: Date(),
            provider: provider,
    ...
    func timeline(
        ...
        in context: Context) async -> Timeline<AgentsBarWidgetEntry>
    {
        let provider = configuration.provider.provider
        let snapshot = WidgetSnapshotStore.load() ?? WidgetPreviewData.emptySnapshot()
        let now = Date()
        let entry = AgentsBarWidgetEntry(date: now, provider: provider, snapshot: snapshot)
        let refresh = BurnDownRefreshSchedule.nextRefresh(snapshot: snapshot, provider: provider, now: now)
        return Timeline(entries: [entry], policy: .after(refresh))
    }
}

struct AgentsBarSwitcherTimelineProvider: TimelineProvider {
    func placeholder(in context: Context) -> AgentsBarSwitcherEntry {
        let snapshot = WidgetPreviewData.snapshot()
        let providers = self.availableProviders(from: snapshot)
        return AgentsBarSwitcherEntry(
            date: Date(),
    ...
    func getSnapshot(in context: Context, completion: @escaping (AgentsBarSwitcherEntry) -> Void) {
        completion(self.makeEntry())
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<AgentsBarSwitcherEntry>) -> Void) {
    ...
    private func makeEntry() -> AgentsBarSwitcherEntry {
        let snapshot = WidgetSnapshotStore.load() ?? WidgetPreviewData.emptySnapshot()
        let providers = self.availableProviders(from: snapshot)
...
struct AgentsBarCompactTimelineProvider: AppIntentTimelineProvider {
    func placeholder(in context: Context) -> AgentsBarCompactEntry {
        AgentsBarCompactEntry(
            date: Date(),
            provider: .codex,
            metric: .credits,
            snapshot: WidgetPreviewData.snapshot())
    ...
    func snapshot(for configuration: CompactMetricSelectionIntent, in context: Context) async -> AgentsBarCompactEntry {
        let provider = configuration.provider.provider
        let metric = configuration.metric
        return AgentsBarCompactEntry(
            date: Date(),
            provider: provider,
            metric: metric,
            snapshot: WidgetSnapshotStore.load() ?? WidgetPreviewData.snapshot())
    ...
    func timeline(
        ...
        in context: Context) async -> Timeline<AgentsBarCompactEntry>
    {
        let provider = configuration.provider.provider
        let metric = configuration.metric
        let snapshot = WidgetSnapshotStore.load() ?? WidgetPreviewData.emptySnapshot()
        let entry = AgentsBarCompactEntry(
            date: Date(),
            provider: provider,
            metric: metric,
            snapshot: snapshot)
