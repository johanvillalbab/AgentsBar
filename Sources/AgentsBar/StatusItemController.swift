...
final class StatusItemController: NSObject, NSMenuDelegate, StatusItemControlling {
    ...
    private nonisolated static let statusItemAccessibilityTitle = "AgentsBar"
    private nonisolated static let debugStatusItemAccessibilityTitle = "AgentsBar Debug"
    private nonisolated static let statusItemAccessibilityIdentifierPrefix = "AgentsBar.StatusItem"
    ...
    let loginLogger = AgentsBarLog.logger(LogCategories.login)
    let menuLogger = AgentsBarLog.logger(LogCategories.app)
    ...
    @objc private func handleProviderConfigDidChange(_ notification: Notification) {
        ...
        {
            if let config = notification.userInfo?["config"] as? AgentsBarConfig {
