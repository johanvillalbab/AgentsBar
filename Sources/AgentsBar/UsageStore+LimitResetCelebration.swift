...
extension UsageStore {
        ...
        observation: LimitResetObservation?)
    {
        ...
        if claudeWeeklyRecoveryPending, wasAboveThreshold {
            AgentsBarLog.logger(LogCategories.confetti).debug(
        ...
        AgentsBarLog.logger(LogCategories.confetti).info(
        ...
        case .session:
            ...
            let event = SessionLimitResetEvent(
                ...
                accountLabel: accountLabel,
                usedPercent: currentUsed)
            NotificationCenter.default.post(name: .agentsbarSessionLimitReset, object: event)
        case .weekly:
            self.emitQuotaResetHook(
                provider: context.provider,
                window: .weekly,
            ...
            let event = WeeklyLimitResetEvent(
                ...
                accountLabel: accountLabel,
                usedPercent: currentUsed)
            NotificationCenter.default.post(name: .agentsbarWeeklyLimitReset, object: event)
        default:
            return
        }
    }
