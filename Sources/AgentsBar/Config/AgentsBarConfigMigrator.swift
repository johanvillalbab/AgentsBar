...
struct AgentsBarConfigMigrator {
    struct LegacyStores {
    ...
    static func loadOrMigrate(
        ...
        stores: LegacyStores) -> AgentsBarConfig
    {
        let log = AgentsBarLog.logger(LogCategories.configMigration)
        let existing = try? configStore.load()
        var config = (existing ?? AgentsBarConfig.makeDefault()).normalized()
    ...
    private static func applyProviderOrder(_ raw: [String], config: AgentsBarConfig) -> AgentsBarConfig {
    ...
    private static func applyProviderToggles(
        ...
        config: AgentsBarConfig) -> AgentsBarConfig
    {
        var updated = config
