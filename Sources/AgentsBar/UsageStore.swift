...
final class UsageStore {
    ...
    @ObservationIgnored let sessionQuotaLogger = AgentsBarLog.logger(LogCategories.sessionQuota)
    @ObservationIgnored let openAIWebLogger = AgentsBarLog.logger(LogCategories.openAIWeb)
    @ObservationIgnored private let tokenCostLogger = AgentsBarLog.logger(LogCategories.tokenCost)
    @ObservationIgnored let augmentLogger = AgentsBarLog.logger(LogCategories.augment)
    @ObservationIgnored let providerLogger = AgentsBarLog.logger(LogCategories.providers)
    @ObservationIgnored let adaptiveRefreshLogger = AgentsBarLog.logger(LogCategories.adaptiveRefresh)
...
extension UsageStore {
    func debugDumpClaude() async {
        ...
        let url = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("agentsbar-claude-probe.txt")
    ...
    func dumpLog(toFileFor provider: UsageProvider) async -> URL? {
        ...
        let filename = "agentsbar-\(provider.rawValue)-probe.txt"
        ...
        fallbackCookieHeader: String) async -> ClaudeDebugLogConfiguration
    {
        await MainActor.run {
            ...
            return ClaudeDebugLogConfiguration(
                runtime: AgentsBarCore.ProviderRuntime.app,
