...
enum AgentsBarLocalizationOverride {
...
private func appLanguageDefaults() -> UserDefaults {
    ...
    return UserDefaults(suiteName: "AgentsBar") ?? .standard
}
...
private func resolvedAppLanguage() -> String {
    if let override = AgentsBarLocalizationOverride.appLanguage {
        return override
    }
...
func agentsBarLocalizationSignature() -> String {
    resolvedAppLanguage()
}

/// Resolving the `.lproj`/resource bundles repeats `Bundle(url:)`/`Bundle(path:)` filesystem lookups,
/// which are surprisingly hot: every `L(…)` and `agentsBarLocalizationSignature()` call runs them, and
...
func agentsBarLocalizationResourceBundle(
    mainBundle: Bundle = .main,
    bundleName: String = "AgentsBar_AgentsBar") -> Bundle
{
    ...
    guard mainBundle === Bundle.main, bundleName == "AgentsBar_AgentsBar" else {
...
private func resolveLocalizedBundle(forLanguage language: String) -> Bundle {
    let resourceBundle = agentsBarLocalizationResourceBundle()
    if !language.isEmpty {
...
func L(_ key: String) -> String {
    let resourceBundle = agentsBarLocalizationResourceBundle()
    return agentsBarLocalizedString(key, bundle: localizedBundle(), resourceBundle: resourceBundle)
}
...
func L(_ key: String, language: String) -> String {
    ...
    return agentsBarLocalizedString(key, bundle: bundle, resourceBundle: resourceBundle)
}

func agentsBarLocalizedLocale() -> Locale {
    let language = resolvedAppLanguage()
...
func agentsBarLocalizedInteger(_ value: Int) -> String {
    value.formatted(.number.locale(agentsBarLocalizedLocale()))
}

func agentsBarLocalizedString(_ key: String, bundle: Bundle, resourceBundle: Bundle) -> String {
...
func resetAgentsBarLocalizationCache() {
    LocalizationBundleCache.reset()
}

#if DEBUG
func agentsBarLocalizedBundleForTesting() -> Bundle {
    localizedBundle()
}

func resetAgentsBarLocalizationCacheForTesting() {
    resetAgentsBarLocalizationCache()
}
#endif
