...
final class OpenAICreditsPurchaseWindowController: NSWindowController, WKNavigationDelegate, WKScriptMessageHandler {
    ...
    private static let logHandlerName = "agentsbarLog"
    private static let debugLogURL = URL(fileURLWithPath: NSTemporaryDirectory())
        .appendingPathComponent("agentsbar-buy-credits.log")
    private static let autoStartScript = """
    (() => {
      if (window.__agentsbarAutoBuyCreditsStarted) return 'already';
      const log = (...args) => {
        try {
          window.webkit?.messageHandlers?.agentsbarLog?.postMessage(args);
        } catch {}
      };
      ...
      const startNextPolling = (initialDelay = 500, interval = 500, maxAttempts = 90) => {
        if (window.__agentsbarNextPolling) return;
        window.__agentsbarNextPolling = true;
      ...
      const observeNextButton = () => {
        if (window.__agentsbarNextObserver || !window.MutationObserver) return;
        const observer = new MutationObserver(() => {
          if (clickNextIfReady(1)) {
            observer.disconnect();
            window.__agentsbarNextObserver = null;
          }
        });
        observer.observe(document.body, { subtree: true, childList: true, attributes: true });
        window.__agentsbarNextObserver = observer;
      };
      ...
      if (findAndClick()) {
        window.__agentsbarAutoBuyCreditsStarted = true;
        startNextPolling();
        observeNextButton();
        logDialogButtons();
        return 'clicked';
    ...
    private let logger = AgentsBarLog.logger(LogCategories.creditsPurchase)
