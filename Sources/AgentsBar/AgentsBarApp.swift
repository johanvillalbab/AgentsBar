...
struct AgentsBarApp: App {
    ...
    init() {
        ...
        let storedLevel = AgentsBarLog.parseLevel(UserDefaults.standard.string(forKey: "debugLogLevel")) ?? .verbose
        let level = AgentsBarLog.parseLevel(env["AGENTSBAR_LOG_LEVEL"]) ?? storedLevel
        AgentsBarLog.bootstrapIfNeeded(.init(
            destination: .oslog(subsystem: "com.steipete.agentsbar"),
            level: level,
            json: false))

        let version = Bundle.main.object(forInfoDictionaryKey: "CFBundleShortVersionString") as? String ?? "unknown"
        let build = Bundle.main.object(forInfoDictionaryKey: "CFBundleVersion") as? String ?? "unknown"
        let gitCommit = Bundle.main.object(forInfoDictionaryKey: "AgentsBarGitCommit") as? String ?? "unknown"
        let buildTimestamp = Bundle.main.object(forInfoDictionaryKey: "AgentsBarBuildTimestamp") as? String ?? "unknown"
        AgentsBarLog.logger(LogCategories.app).info(
            "AgentsBar starting",
            metadata: [
                "version": version,
                "build": build,
        ...
        AgentsBarLog.setLogLevel(settings.debugLogLevel)
    ...
    var body: some Scene {
        ...
        WindowGroup("AgentsBarLifecycleKeepalive") {
            HiddenWindowView()
        }
    ...
    private func openSettings(pane: SettingsPane) {
        ...
        let logger = AgentsBarLog.logger(LogCategories.app)
        switch outcome {
        case .preferred:
            break
        case .fallback:
...
private func makeUpdaterController() -> UpdaterProviding {
    ...
    if InstallOrigin.isHomebrewCask(appBundleURL: bundleURL) {
        return DisabledUpdaterController(
            unavailableReason: "Updates managed by Homebrew. Run: brew upgrade --cask steipete/tap/agentsbar")
    }
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private let confettiLogger = AgentsBarLog.logger(LogCategories.confetti)
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if !self.hasInstalledLimitResetObservers {
            NotificationCenter.default.addObserver(
                ...
                name: .agentsbarSessionLimitReset,
                object: nil)
            NotificationCenter.default.addObserver(
                self,
                selector: #selector(self.handleWeeklyLimitResetNotification(_:)),
                name: .agentsbarWeeklyLimitReset,
                object: nil)
    ...
    private func ensureStatusController() {
        ...
        AgentsBarLog.logger(LogCategories.app)
    ...
    private func installDebugMemoryPressureObserverIfNeeded() {
        ...
        self.debugMemoryPressureObserver = DistributedNotificationCenter.default().addObserver(
            forName: .agentsbarDebugSimulateMemoryPressure,
            object: nil,
            queue: .main)
    ...
    private func handleDebugMemoryPressureNotification(rawLevel: String?, shouldSeedCaches: Bool) {
        ...
        AgentsBarLog.logger(LogCategories.memoryPressure).info(
