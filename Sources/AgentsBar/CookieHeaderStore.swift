...
struct KeychainCookieHeaderStore: CookieHeaderStoring {
    private static let log = AgentsBarLog.logger(LogCategories.cookieHeaderStore)
    ...
    func loadCookieHeader() throws -> String? {
        ...
        Self.cacheLock.unlock()
        if let header = try self.loadCookieHeader(service: self.service) {
            Self.cacheLock.lock()
            Self.cache[self.account] = CachedValue(value: header, timestamp: Date())
            Self.cacheLock.unlock()
            return header
        }
        if let header = try self.loadCookieHeader(service: self.legacyService) {
            Self.cacheLock.lock()
            Self.cache[self.account] = CachedValue(value: header, timestamp: Date())
            Self.cacheLock.unlock()
            return header
    ...
    private func loadCookieHeader(service: String) throws -> String? {
