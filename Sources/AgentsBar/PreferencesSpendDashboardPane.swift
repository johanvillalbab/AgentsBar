...
func spendDashboardDayRangeText(_ days: Int) -> String {
    ...
    default: return agentsBarLocalizedInteger(days)
    }
    return template.replacingOccurrences(
        of: String(days),
        with: agentsBarLocalizedInteger(days))
}

func spendDashboardRankText(_ rank: Int) -> String {
    "#\(agentsBarLocalizedInteger(rank))"
}

func spendDashboardRefreshFailureText(_ count: Int) -> String {
    "\(L("Refresh failures")): \(agentsBarLocalizedInteger(count))"
}

func spendDashboardCoverageText(covered: Int, requested: Int) -> String {
    "\(L("Coverage")): \(agentsBarLocalizedInteger(covered)) / \(agentsBarLocalizedInteger(requested))"
}
            ...
            SpendDashboardPanel {
                HStack(spacing: 24) {
                    ...
                    SpendSummaryValue(
                        ...
                        value: agentsBarLocalizedInteger(self.group.providers.count))
                    Spacer()
                }
            }
...
private struct SpendDailyChart: View {
    ...
    private func pointAccessibilityLabel(_ point: SpendDashboardModel.DailyPoint) -> String {
        let day = point.day.formatted(
            .dateTime.month(.abbreviated).day().locale(agentsBarLocalizedLocale()))
