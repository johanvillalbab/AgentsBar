...
extension SettingsStore {
    private func updateConfig(
        ...
        mutate: (inout AgentsBarConfig) -> Void)
    {
    ...
    func updateProviderTokenAccounts(_ accounts: [UsageProvider: ProviderTokenAccountData]) {
        let summary = accounts
            ...
            .joined(separator: ",")
        AgentsBarLog.logger(LogCategories.tokenAccounts).info(
    ...
    func reloadConfig(reason: String, affectsBackgroundWork: Bool? = nil) {
        ...
        do {
            ...
            AgentsBarLog.logger(LogCategories.configStore).error("Failed to reload config: \(error)")
        }
    }

    func applyExternalConfig(
        _ config: AgentsBarConfig,
        reason: String,
    ...
    private static func configChangeAffectsBackgroundWork(
        from previous: AgentsBarConfig,
        to current: AgentsBarConfig) -> Bool
    {
    ...
    private static func orderIndependentConfigData(_ config: AgentsBarConfig) -> Data? {
    ...
    private func bumpConfigRevision(_ context: ConfigChangeContext) {
        ...
        AgentsBarLog.logger(LogCategories.settings)
            .debug(
    ...
    func schedulePersistConfig() {
        ...
        if Self.isRunningTests {
            do {
                ...
                AgentsBarLog.logger(LogCategories.configStore).error("Failed to persist config: \(error)")
            }
            return
        }
