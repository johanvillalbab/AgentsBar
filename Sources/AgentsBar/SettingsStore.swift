...
final class SettingsStore {
    ...
    @ObservationIgnored let configStore: AgentsBarConfigStore
    @ObservationIgnored let antigravityOAuthCredentialsStore: AntigravityOAuthCredentialsStore
    @ObservationIgnored var config: AgentsBarConfig
    ...
    init(
        ...
        configStore: AgentsBarConfigStore = AgentsBarConfigStore(),
        ...
        performInitialProviderDetection: Bool = !SettingsStore.isRunningTests)
    {
        ...
        if !Self.isRunningTests {
            AgentsBarLog.logger(LogCategories.settings).info(
                "App group resolved",
                metadata: [
                    "groupID": appGroupID,
        ...
        let legacyStores = AgentsBarConfigMigrator.LegacyStores(
        ...
        let config = AgentsBarConfigMigrator.loadOrMigrate(
        ...
        AgentsBarLog.setFileLoggingEnabled(self.debugFileLoggingEnabled)
...
extension SettingsStore {
        ...
        hadPreviousInstallationState: Bool) -> SettingsDefaultsState
    {
        ...
        let debugLogLevelRaw = userDefaults.string(forKey: "debugLogLevel") ?? AgentsBarLog.Level.verbose.rawValue
...
extension SettingsStore {
    var configSnapshot: AgentsBarConfig {
        _ = self.configRevision
        return self.config
    }

    func updateProviderState(config: AgentsBarConfig) {
    ...
    func setProviderEnabled(provider: UsageProvider, metadata _: ProviderMetadata, enabled: Bool) {
        AgentsBarLog.logger(LogCategories.settings).debug(
